...
public:
  ...
  void ToggleLayout()
  {
    ...
    const uint32_t count      = mCard.GetChildCount(ChildScopePolicy::LAYOUT_CHILDREN);
    for(uint32_t i = 0; i < count; ++i)
    {
      View::DownCast(mCard.GetChildAt(i, ChildScopePolicy::LAYOUT_CHILDREN)).SetRequestedHeight(itemHeight);
    }
  }
