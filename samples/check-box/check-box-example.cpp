...
// The control does NOT define a default size, so every CheckBox below is sized by the
// application (SetRequestedWidth/Height); the icon fills the content height.
//
...
namespace
{
constexpr float CHECKBOX_HEIGHT = 48.0f;

// Capture-less icon generator for CheckBoxStyle::SetIconGenerator(). The app supplies the
// glyph url together with the EXPLICIT integer frame ranges played on select / deselect. Here
// we reuse the shipped asset (via the default style's icon) and pass the same [0,30] / [30,48]
// segments; a real app can swap in its own asset. Must be a free function
// (IconGenerator = Ui::Callback<ISelectableImage()>).
ISelectableImage MakeSampleIcon()
{
  // Recover the shipped checkbox.json url from the default icon's drawing view.
  ISelectableImage defaultIcon = CheckBoxStyle::Default().CreateIcon();
