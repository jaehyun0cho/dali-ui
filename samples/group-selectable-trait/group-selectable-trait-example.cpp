...
/**
 * GroupSelectableTrait sample: declarative single-selection (radio-button style)
 * grouping of plain circular Views, stacked vertically.
 *
 * Each option is an ordinary View made round with a relative corner radius. Each
 * option declares its intent to be group-selectable via View::AsGroupSelectable(),
 * and relies on parent auto-grouping (on by default): once added to the shared
 * stack and connected to a scene, each option joins the SelectionGroup associated
 * with its parent View, which enforces that at most one is selected at a time.
 * Toggle-by-click is on by default, so tapping a circle selects it and deselects
 * the previously selected one; re-tapping the selected circle is a no-op.
 * SelectedMemberChangedSignal recolours the previous and current options and
 * updates the top label to report which option is selected.
 *
...
public:
  ...
  void Create(Application application)
  {
    ...
    for(int i = 0; i < OPTION_COUNT; ++i)
    {
      ...
      // Declare the option group-selectable. With parent auto-grouping (the
      // default), the option joins the SelectionGroup of its parent View (the
      // shared stack) on scene connection, so tapping the circle selects it
      // within that group. This also makes the View selectable (AsGroupSelectable
      // composes a SelectableTrait), with toggle-by-click enabled by default.
    ...
    // The options share the root stack as their parent, so they all auto-join the
    // root's SelectionGroup. Look it up by the same parent View to drive selection
    // and observe changes. (After window.Add(root), the children are scene-connected
    // and have joined.)
...
int DALI_EXPORT_API main(int argc, char** argv)
{
  ...
  // Initialise the UI configuration before any trait is created. SelectionGroup
