 ...
 * plain circular Views, stacked vertically, driven by an explicit SelectionGroup.
 *
 * Each option is an ordinary View made round with a relative corner radius. A
 * single SelectionGroup is created with SelectionGroup::New(), and each option is
 * bound to it with SelectionGroup::Add(view). Add() makes the View group-selectable
 * (it composes a SelectableTrait, which implies an InteractiveTrait) and installs
 * the group's select-only click wiring, so:
...
public:
  ...
  void Create(Application application)
  {
    ...
    // The explicit single-selection controller. Members are added to it below.
    mGroup = SelectionGroup::New();
    ...
    for(int i = 0; i < OPTION_COUNT; ++i)
    {
      ...
      // Explicit membership: Add() makes the option group-selectable and binds it
      // to this group, installing the select-only click wiring (tap = select,
      // re-tap = no-op) and the radio accessibility costume.
      mGroup.Add(circle);
    ...
    // Start with the first option selected. The current SelectionGroup API has no
    // group-level setter, so programmatic selection goes through the member's
    // SelectableTrait; the group observes the change and enforces exclusivity.
...
int DALI_EXPORT_API main(int argc, char** argv)
{
  ...
  // membership creates an InteractiveTrait, which reads UiConfig (e.g. the
