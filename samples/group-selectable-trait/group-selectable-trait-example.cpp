 ...
 * plain circular Views, stacked vertically, formed declaratively by a shared group
 * name and observed through a SelectionGroup handle.
 *
 * Each option is an ordinary View made round with a relative corner radius. Each
 * option declares its membership with circle.AsGroupSelectable().SetGroupName(name):
 * giving every option the SAME name binds them into one named SelectionGroup
 * (cross-parent, persistent). AsGroupSelectable() makes the View group-selectable
 * (it composes a SelectableTrait, which implies an InteractiveTrait), and joining
 * the named group installs the select-only click wiring, so:
 *   - tapping a circle selects it and auto-deselects the previously selected one;
 *   - re-tapping the selected circle is a no-op (true radio semantics);
 *   - a gesture can never empty the group.
 *
 * The shared group is OBTAINED (not created) with SelectionGroup::Find(name).
 ...
 *
 * (An alternative to a shared name is parent auto-grouping: AsGroupSelectable()
 * children added under one on-scene View parent auto-join that parent's group with
 * no name at all. This sample uses the named form so the group is easy to observe.)
 *
 * Keys:
...
public:
  ...
  void Create(Application application)
  {
    ...
    for(int i = 0; i < OPTION_COUNT; ++i)
    {
      ...
      // Declarative membership: giving every option the same group name binds it to
      // the shared named group, installing the select-only click wiring (tap =
      // select, re-tap = no-op) and the radio accessibility costume.
      circle.AsGroupSelectable().SetGroupName(GROUP_NAME);
    ...
    // the same group the options joined above.
