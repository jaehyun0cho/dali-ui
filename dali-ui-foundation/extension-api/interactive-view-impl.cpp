...
Signal<void(View, bool, InputEvent)>& InteractiveViewImpl::HoveredChangedSignal()
{
  return GetInteractiveTrait().HoveredChangedSignal();
}
...
bool InteractiveViewImpl::IsHovered() const
{
  return GetInteractiveTrait().IsHovered();
}
