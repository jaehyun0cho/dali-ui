...
/**
 * @brief SelectableGroup is a logical controller that enforces single selection
 ...
 * are Views backed by a GroupSelectableTrait, which Add() attaches automatically.
 *
...
public: // Creation & Destruction
  /**
   * @brief Creates an uninitialized SelectableGroup handle.
   */
  SelectableGroup();

  /**
   * @brief Creates a new exclusive SelectableGroup.
   *
   * The new group allows empty selection by default.
   *
   * @return A handle to a newly created SelectableGroup
   */
  static SelectableGroup New();

  /**
   * @brief Downcasts a handle to a SelectableGroup handle.
   *
   * @param[in] handle Handle to an object
   * @return A handle to SelectableGroup or an uninitialized handle
   */
   ...
   * @param[in] selectableGroup Handle to copy
   */
  SelectableGroup(const SelectableGroup& selectableGroup);

  /**
   * @brief Move constructor.
   * @param[in] rhs Handle to move
   */
  SelectableGroup(SelectableGroup&& rhs) noexcept;
   ...
   */
  SelectableGroup& operator=(const SelectableGroup& rhs);
   ...
   * @return Reference to this
   */
  SelectableGroup& operator=(SelectableGroup&& rhs) noexcept;
...
public: // Signals
   ...
   * the input event that originated the change, or InputEvent::None() for
...
public: // API
   ...
   *
   * If the View has no selectable trait, a GroupSelectableTrait is created and
   * attached. If it already has a GroupSelectableTrait (possibly from another
   * group), it is rebound to this group. If it already has a plain
   * SelectableTrait, the View is left unchanged and the call fails (an error is
   * logged). A plain SelectableTrait is never replaced because that would lose its
   * selected state and signal subscribers; therefore add a View to its group BEFORE
   * calling View::AsSelectable() on it.
   *
   ...
   *         the View is empty, already has a plain SelectableTrait, or a
   *         selection transition is in progress
   */
...
public: // Not intended for application developers
   ...
   */
  explicit SelectableGroup(Integration::SelectableGroupImpl* implementation);
};
