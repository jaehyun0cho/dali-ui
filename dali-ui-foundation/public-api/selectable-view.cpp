...
{
  return GetImpl(*this).EnsureSelectableTrait().SelectionChangedSignal();
}

bool SelectableView::IsSelected() const
{
  return const_cast<SelectableViewImpl&>(GetImpl(*this)).EnsureSelectableTrait().IsSelected();
}

void SelectableView::SetSelected(bool selected)
{
  GetImpl(*this).EnsureSelectableTrait().SetSelected(selected);
}

bool SelectableView::IsToggleByClickEnabled() const
{
  return const_cast<SelectableViewImpl&>(GetImpl(*this)).EnsureSelectableTrait().IsToggleByClickEnabled();
}

void SelectableView::EnableToggleByClick(bool enabled)
{
  GetImpl(*this).EnsureSelectableTrait().EnableToggleByClick(enabled);
}
