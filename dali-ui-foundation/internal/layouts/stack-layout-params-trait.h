 ...
 */
class StackLayoutParamsTrait final : public TraitObject
{
public:
  explicit StackLayoutParamsTrait(const StackLayoutParams& params)
  : mWeight(params.GetWeight()),
    mAlignment(params.GetAlignment())
  {
  }

  void CopyTo(StackLayoutParams& params) const
  {
    params.SetWeight(mWeight)
   ...
   */
  static const StackLayoutParamsTrait* Get(const ViewImpl& viewImpl)
  {
    IntrusivePtr<TraitObject> object = Integration::View::GetTrait(viewImpl, Integration::ReservedTraitId::STACK_LAYOUT_PARAMS);
    DALI_ASSERT_DEBUG(!object || (dynamic_cast<const StackLayoutParamsTrait*>(object.Get()) && "STACK_LAYOUT_PARAMS trait must be a StackLayoutParamsTrait"));
    return object ? static_cast<const StackLayoutParamsTrait*>(object.Get()) : nullptr;
  }

protected:
  ~StackLayoutParamsTrait() override = default;

private:
  const float           mWeight;
  const LayoutAlignment mAlignment;
};
