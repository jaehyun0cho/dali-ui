 ...
 */
class FlexLayoutParamsTrait final : public TraitObject
{
public:
  explicit FlexLayoutParamsTrait(const FlexLayoutParams& params)
  ...
  void CopyTo(FlexLayoutParams& params) const
  {
    params.SetFlexGrow(mFlexGrow)
      .SetFlexShrink(mFlexShrink)
      .SetFlexBasis(mFlexBasis)
      .SetAlignSelf(mAlignSelf);
  }
   ...
   */
  static const FlexLayoutParamsTrait* Get(const ViewImpl& viewImpl)
  {
    IntrusivePtr<TraitObject> object = Integration::View::GetTrait(viewImpl, Integration::ReservedTraitId::FLEX_LAYOUT_PARAMS);
    DALI_ASSERT_DEBUG(!object || (dynamic_cast<const FlexLayoutParamsTrait*>(object.Get()) && "FLEX_LAYOUT_PARAMS trait must be a FlexLayoutParamsTrait"));
    return object ? static_cast<const FlexLayoutParamsTrait*>(object.Get()) : nullptr;
  }

protected:
  ~FlexLayoutParamsTrait() override = default;

private:
  const float     mFlexGrow;
  const float     mFlexShrink;
  const float     mFlexBasis;
  const FlexAlign mAlignSelf;
};
