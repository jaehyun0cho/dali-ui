 ...
 *
 * An epoch is what lets the second and subsequent walks be skipped SAFELY. Each view
 * records the epoch in which its walk completed; a later invalidation may skip the
 * walk only while that record still matches the current epoch, which is exactly the
...
{
/**
 * @brief Returns the current invalidation epoch.
 *
 * Never returns 0, so 0 is usable as a "never propagated" initial value.
 *
 * @return The current epoch
 */
DALI_UI_API uint32_t CurrentEpoch();

/**
 * @brief Ends the current epoch and starts a new one.
 *
 ...
 *
 * Every recorded epoch becomes stale at that moment, so the next invalidation on any
 ...
 * with the current epoch after 2^32 bumps, a collision merely coalesces one walk that
 * should have run, and the next pass re-ends the epoch. Skipping 0 on wrap keeps the
 * "never propagated" value unreachable.
 */
DALI_UI_API void BeginNewEpoch();
