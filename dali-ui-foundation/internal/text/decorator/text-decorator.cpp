...
struct Decorator::Impl : public ConnectionTracker
{
  ...
  void SetSelectionHandleMarkerSize(HandleImpl& handle)
  {
    ...
    {
      handle.markerActor.SetRequestedWidth(0.0f);
      handle.markerActor.SetRequestedHeight(handle.lineHeight);
    }
  }
  ...
  void ApplyDisplacement(HandleImpl& handle, HandleType type, float yLocalPosition)
  {
    if(handle.actor)
    {
      ...
      handle.actor.SetRequestedX(x);
      handle.actor.SetRequestedY(y);
