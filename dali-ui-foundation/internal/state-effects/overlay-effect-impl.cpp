...
void OverlayEffectData::ApplyRecoil(View overlayTarget, RecoilScope scope)
{
  ...
  {
    const uint32_t childCount = overlayTarget.GetChildCount(ChildScopePolicy::LAYOUT_CHILDREN);
    ...
    {
      addRecoilTarget(View::DownCast(overlayTarget.GetChildAt(i, ChildScopePolicy::LAYOUT_CHILDREN)));
    }
  }
