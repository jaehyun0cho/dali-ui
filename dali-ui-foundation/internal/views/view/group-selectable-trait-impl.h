...
public: // SelectionGroupImpl collaboration
   ...
   *
   * Called by SelectionGroupImpl::Remove(). Performs the teardown in order: structural
   ...
   * fully detached plain Selectable when the callback runs, so a re-add via group.Add()
   * from the callback is not lost. The member's own selected state is preserved.
   */
  void LeaveGroup();
