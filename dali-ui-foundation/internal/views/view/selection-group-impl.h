...
public: // Trait collaboration (called by GroupSelectableTraitImpl)
  ...
  /**
   * @brief Unregisters a member from the group.
   *
   * Called by GroupSelectableTraitImpl on Remove, detach, or View destruction. If the
   * member was the current winner, the group becomes empty (no promotion). It emits
   * SelectedMemberChangedSignal(previous, empty) once IF the previous View is still
   * resolvable (Remove(winner), cross-group move); during View destruction the member's
   * View already resolves empty, so the emit is skipped. Keyed on the stable
   ...
   */
  void UnregisterMember(GroupSelectableTraitImpl* member);
