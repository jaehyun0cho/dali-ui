...
bool ViewDataImpl::CanUseVisuals() const
{
  return AreVisualsEnabled() && (mViewImpl.GetOwner() != nullptr);
}
...
void ViewDataImpl::ReplayArrangeSubtreeFromCache(bool mirrorUnderParentRtl, float parentArrangedWidth, const ViewImpl* mirrorParent)
{
  ...
  ApplySelfBoundsIfChanged(applied, mirrorUnderParentRtl ? mirrorParent : nullptr);
  ...
  if(!mChildren.Empty())
  {
    ...
    for(auto& childView : childSnapshot)
    {
      ...
      childData.ReplayArrangeSubtreeFromCache(mirrorChild, cached.width, &mViewImpl);
    }
  }
...
LayoutRect ViewDataImpl::ArrangeImpl(const LayoutRect& bounds, bool frameworkLayoutRootPass)
{
     ...
     (mChildren.Empty() || CanReplayArrangeSubtreeFromCache()))
  {
    ...
    ReplayArrangeSubtreeFromCache(false, 0.0f, nullptr);
...
void ViewDataImpl::ApplyLayoutDirection(float parentWidth)
{
  ...
  for(auto& childView : childSnapshot)
  {
    ...
    {
      DALI_UI_LAYOUT_TEST_EVENT(Integration::LayoutTestDiagnostics::EventKind::GEOMETRY_WRITE, &childData.mViewImpl, &mViewImpl, 0u, mirrored);
...
void ViewDataImpl::ApplySelfBoundsIfChanged(const LayoutRect& bounds, const ViewImpl* mirrorSource)
{
  Actor self = mViewImpl.Self();
  ...
  //
  // @p mirrorSource is the parent whose right-to-left mirror is fused into the x below, or
  // nullptr when x is plain logical geometry. It only attributes the observation: the MISS
  // path's separate mirror write names the same parent, so a reader sees one shape either way.
  if(self.GetProperty<float>(Actor::Property::POSITION_X) != bounds.x)
  {
    DALI_UI_LAYOUT_TEST_EVENT(Integration::LayoutTestDiagnostics::EventKind::GEOMETRY_WRITE, &mViewImpl, mirrorSource, 0u, bounds.x);
...
Ui::Visual::ResourceStatus ViewDataImpl::GetVisualResourceStatus(Property::Index index) const
{
  ...
  // FindVisual-miss fallback. Only a DISABLE_VISUALS view -- or a query made before the
  // owner handle exists, when the pre-lazy code had no context either -- keeps the READY
  // answer.
  return CanUseVisuals() ? Ui::Visual::ResourceStatus::PREPARING : Ui::Visual::ResourceStatus::READY;
}
  ...
  if(viewImplPtr)
  {
    ...
    switch(index)
    {
      ...
      case Ui::View::Property::CORNER_RADIUS:
      {
        ...
        if(value.Get(radius))
        {
          // Gated on CanUseVisuals(), not on the context already existing:
          ...
          if(DALI_LIKELY(viewImpl.GetViewDataImpl().CanUseVisuals()))
          {
...
{
  DALI_ASSERT_ALWAYS(CanUseVisuals() && "VisualEventSignal needs visuals enabled and an owned view (after handle construction)!!");
...
void ViewDataImpl::ClearInnerShadow()
{
  ...
  // Trigger a size negotiation request that may be needed when unregistering a visual.
  // Gated on CanUseVisuals(), not on the lazy context: before the context became lazy
  // every enabled view had one from Initialize() on, so an owned, enabled view ALWAYS
  // issued this request here even with nothing registered -- and a visuals-disabled
  // view, or a call from the impl constructor, never did.
  if(CanUseVisuals())
  {
    if(Integration::SizeNegotiatedViewImpl* sizeNegotiatedViewImpl = dynamic_cast<Integration::SizeNegotiatedViewImpl*>(&mViewImpl))
    {
...
{
  // CanUseVisuals(), NOT mVisualData, and deliberately no EnsureVisualData() here:
  // registering must not allocate. The predicate keeps the pre-lazy registration TIMING
  // exactly -- owned and enabled implies registered on the first size/scale change,
  // whether or not a visual exists yet -- because before lazy creation an enabled view
  // always had a context once its handle existed and so always registered here.
  //
