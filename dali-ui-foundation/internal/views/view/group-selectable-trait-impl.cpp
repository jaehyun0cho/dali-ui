...
void GroupSelectableTraitImpl::LeaveGroup()
{
  ...
  // Drop the group binding FIRST: clears mGroup and calls UnregisterMember, which now
  // emits SelectedMemberChangedSignal(previous, empty) if this member was the winner
  // (FIX-2). Done before reverting wiring so the selection observer (which no-ops once
  // mGroup is null) cannot re-enter group arbitration during teardown.
  UnbindFromGroup();

  if(mAttached)
  {
    // Revert the membership-gated interaction: disconnect this trait's own
    // OnClickedForSelect and restore the pre-join toggle-by-click value, so the member
    // behaves once again as a plain Selectable. THEN restore accessibility (role + the
    // CHECKED bit per the restored role, FIX-5).
    RevertInteraction();
