...
// with the segment layout from its markers — select plays [0,30] (check + settle), deselect
// plays [30,48]. Must be a free function (IconGenerator = Ui::Callback<ISelectableImage()>).
...
: mMinimumWidth(0.0f),  // no default: the app sizes the control
  ...
  mIconGenerator(std::make_shared<CheckBoxStyle::IconGenerator>(
    CheckBoxStyle::IconGenerator::New(&MakeDefaultCheckBoxIcon))), // checkbox.json + [0,30]/[30,48]
