...
View TestCreateItemView(uint32_t)
{
  ++gCreatedViews;
  View item = View::New();
...
void TestBindItemView(View, uint32_t position)
{
  ++gBoundViews;
  gBoundPositions.push_back(position);
}

void TestItemViewRecycled(View, uint32_t)
{
  ++gRecycledViews;
}
...
// A ready-to-drive recycler: adapter and layouter are owned by the caller because
// ItemAdapter is non-copyable and must outlive the RecyclerView.
                           ...
                           LinearItemsLayouter& layouter, bool connectRecycled = false)
{
  ...
  adapter.CreateItemViewSignal().Connect(&TestCreateItemView);
  adapter.BindItemViewSignal().Connect(&TestBindItemView);
  if(connectRecycled)
  {
    adapter.ItemViewRecycledSignal().Connect(&TestItemViewRecycled);
  }
