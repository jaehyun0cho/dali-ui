...
int UtcDaliLayoutTransitionNewSuppressedP(void)
{
  UiTestApplication application;
  LayoutTransition  suppressed = LayoutTransition::NewSuppressed();
  DALI_TEST_CHECK(suppressed);

  View        spacer, a, b;
  StackLayout parent = MakeSettledStack(application, spacer, a, b);
  parent.SetLayoutTransition(MakeChangeTiming(0.4f));
  b.SetSelfLayoutTransition(suppressed);
  ...
  // a is mid-flight, b already snapped to its new bounds.
  const float aMid = a.GetCurrentProperty<float>(Actor::Property::POSITION_Y);
  DALI_TEST_CHECK(aMid > 5.0f);
...
int UtcDaliLayoutTransitionSelfSuppressedChangeSnapsP(void)
{
  // NewSuppressed() as the self handle: CHANGE snaps while the sibling animates.
  UiTestApplication application;
  View              spacer, a, b;
  StackLayout       parent = MakeSettledStack(application, spacer, a, b);

  parent.SetLayoutTransition(MakeChangeTiming(0.4f));
  b.SetSelfLayoutTransition(LayoutTransition::NewSuppressed());
...
int UtcDaliLayoutTransitionSelfSuppressedExitUnparentsImmediatelyP(void)
{
  // The documented opt-out: the parent's EXIT effect is not consulted at all.
  UiTestApplication application;
  ResetSelfCaptures();
  ...
  child.SetSelfLayoutTransition(LayoutTransition::NewSuppressed());
...
int UtcDaliLayoutTransitionSelfSuppressedEnterIgnoredP(void)
{
...
int UtcDaliLayoutTransitionSelfRemoveAllHonoursPerChildP(void)
{
  ...
  // decision applies: x defers on its own long EXIT, y opts out and goes now.
  UiTestApplication application;
  ResetSelfCaptures();
  ...
  y.SetSelfLayoutTransition(LayoutTransition::NewSuppressed());
