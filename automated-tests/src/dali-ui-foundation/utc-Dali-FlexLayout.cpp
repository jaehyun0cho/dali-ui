...
int UtcDaliFlexLayoutParamsValueSemanticsP(void)
{
  ...
  FlexLayoutParams moveSource = FlexLayoutParams::New()
                                  .SetFlexGrow(2.0f)
                                  .SetFlexShrink(0.25f)
                                  .SetFlexBasis(80.0f)
                                  .SetAlignSelf(FlexAlign::FLEX_END);
  FlexLayoutParams moveConstructed(std::move(moveSource));
  DALI_TEST_EQUALS(moveConstructed.GetFlexGrow(), 2.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(moveConstructed.GetAlignSelf(), FlexAlign::FLEX_END, TEST_LOCATION);
  (void)moveSource.GetFlexGrow();
  moveSource.SetFlexGrow(3.0f);
  DALI_TEST_EQUALS(moveSource.GetFlexGrow(), 3.0f, TEST_LOCATION);
  moveSource = FlexLayoutParams::New().SetFlexGrow(4.0f);
  DALI_TEST_EQUALS(moveSource.GetFlexGrow(), 4.0f, TEST_LOCATION);

  FlexLayoutParams moveAssigned;
  moveAssigned = std::move(moveConstructed);
  DALI_TEST_EQUALS(moveAssigned.GetFlexBasis(), 80.0f, TEST_LOCATION);
  (void)moveConstructed.GetFlexGrow();
  moveConstructed.SetFlexGrow(5.0f);
  DALI_TEST_EQUALS(moveConstructed.GetFlexGrow(), 5.0f, TEST_LOCATION);
  ...
  auto convenientB = b.GetLayoutParamsOrDefault<FlexLayoutParams>();
  DALI_TEST_EQUALS(convenientB.GetFlexGrow(), 1.0f, TEST_LOCATION);
  convenientB.SetFlexGrow(9.0f);
  DALI_TEST_EQUALS(b.GetLayoutParamsOrDefault<FlexLayoutParams>().GetFlexGrow(), 1.0f, TEST_LOCATION);

  auto defaultParams = empty.GetLayoutParamsOrDefault<FlexLayoutParams>();
  DALI_TEST_EQUALS(defaultParams.GetFlexGrow(), 0.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetFlexShrink(), 1.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetFlexBasis(), WRAP_CONTENT, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetAlignSelf(), FlexAlign::AUTO, TEST_LOCATION);
