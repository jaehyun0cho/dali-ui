...
int UtcDaliAbsoluteLayoutParamsValueSemanticsP(void)
{
  ...
  AbsoluteLayoutParams moveSource = AbsoluteLayoutParams::New().SetBounds(boundsA).SetFlags(AbsoluteLayoutFlags::ALL);
  AbsoluteLayoutParams moveConstructed(std::move(moveSource));
  DALI_TEST_EQUALS(moveConstructed.GetBounds().GetX(), boundsA.GetX(), TEST_LOCATION);
  DALI_TEST_EQUALS(static_cast<uint8_t>(moveConstructed.GetFlags()), static_cast<uint8_t>(AbsoluteLayoutFlags::ALL), TEST_LOCATION);
  (void)moveSource.GetX();
  moveSource.SetX(11.0f);
  DALI_TEST_EQUALS(moveSource.GetX(), 11.0f, TEST_LOCATION);
  moveSource = AbsoluteLayoutParams::New().SetX(12.0f);
  DALI_TEST_EQUALS(moveSource.GetX(), 12.0f, TEST_LOCATION);

  AbsoluteLayoutParams moveAssigned;
  moveAssigned = std::move(moveConstructed);
  DALI_TEST_EQUALS(moveAssigned.GetBounds().GetX(), boundsA.GetX(), TEST_LOCATION);
  (void)moveConstructed.GetX();
  moveConstructed.SetX(13.0f);
  DALI_TEST_EQUALS(moveConstructed.GetX(), 13.0f, TEST_LOCATION);
  ...
  auto convenientB = b.GetLayoutParamsOrDefault<AbsoluteLayoutParams>();
  DALI_TEST_EQUALS(convenientB.GetBounds().GetX(), boundsA.GetX(), TEST_LOCATION);
  convenientB.SetX(99.0f);
  DALI_TEST_EQUALS(b.GetLayoutParamsOrDefault<AbsoluteLayoutParams>().GetX(), boundsA.GetX(), TEST_LOCATION);

  auto defaultParams = empty.GetLayoutParamsOrDefault<AbsoluteLayoutParams>();
  DALI_TEST_EQUALS(defaultParams.GetX(), 0.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetY(), 0.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetWidth(), -1.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(defaultParams.GetHeight(), -1.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(static_cast<uint8_t>(defaultParams.GetFlags()), static_cast<uint8_t>(AbsoluteLayoutFlags::NONE), TEST_LOCATION);
  ...
  a.SetLayoutParams(FlexLayoutParams::New().SetFlexGrow(4.0f));
  a.SetLayoutParams(GridLayoutParams::New().SetRow(5u));
  a.SetLayoutParams(StackLayoutParams::New().SetWeight(6.0f));
  DALI_TEST_EQUALS(a.GetLayoutParamsOrDefault<AbsoluteLayoutParams>().GetBounds().GetX(), boundsB.GetX(), TEST_LOCATION);
  DALI_TEST_EQUALS(a.GetLayoutParamsOrDefault<FlexLayoutParams>().GetFlexGrow(), 4.0f, TEST_LOCATION);
  DALI_TEST_EQUALS(a.GetLayoutParamsOrDefault<GridLayoutParams>().GetRow(), 5u, TEST_LOCATION);
  DALI_TEST_EQUALS(a.GetLayoutParamsOrDefault<StackLayoutParams>().GetWeight(), 6.0f, TEST_LOCATION);
  END_TEST;
}
