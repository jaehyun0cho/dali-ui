...
// B3-1-grid: a spanning child across an AUTO + ABSOLUTE column pair must grow
...
// B3-1-grid documented contract: STAR tracks are NOT pre-seeded in the span
...
int UtcDaliGridLayoutColumnSpanGrowsAutoTracksP(void)
{
  ...
  // clipped (CSS Grid Layout 11.5.1). The auto columns have no other content,
  // so their content floor is zero and the spanning child's width is shared
  // equally across both columns.
  UiTestApplication application;
  GridLayout        layout = GridLayout::New();
  layout.AddRowDefinition(GridLength::Auto());
  layout.AddColumnDefinition(GridLength::Auto());
