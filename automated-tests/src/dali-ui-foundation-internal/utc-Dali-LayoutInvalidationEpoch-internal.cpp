...
void utc_dali_layout_invalidation_epoch_internal_startup(void)
{
  test_return_value = TET_UNDEF;
}

void utc_dali_layout_invalidation_epoch_internal_cleanup(void)
{
  test_return_value = TET_PASS;
}

// White-box coverage for the invalidation PROPAGATION epoch.
//
...
LayoutRect StagedRootArrange(View, const LayoutRect& bounds)
{
  ...
  {
    // (a) First invalidation. Walks leaf -> mid -> root and records the epoch on all
    ...
    // (c) Second invalidation of the same leaf, in the same epoch. If the epoch
...
int UtcDaliLayoutInvalidationEpochRecordedOnPropagationP(void)
{
  UiTestApplication application;
  tet_infoline("A view's propagation record is 0 until it propagates, then names the current epoch");

  Chain chain = BuildChain(application);

  // A settled view has propagated at least once (during construction), and the drain
  // that settled it started a new epoch, so every record is now stale.
  const uint32_t settledEpoch = LIE::CurrentEpoch();
  DALI_TEST_CHECK(DataOf(chain.leaf).GetMeasurePropagationEpoch() != settledEpoch);

  chain.leaf.InvalidateMeasure();

  // The walk ran and recorded itself at every level it passed through.
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetMeasurePropagationEpoch(), settledEpoch, TEST_LOCATION);
  DALI_TEST_EQUALS(DataOf(chain.mid).GetMeasurePropagationEpoch(), settledEpoch, TEST_LOCATION);
  DALI_TEST_EQUALS(DataOf(chain.root).GetMeasurePropagationEpoch(), settledEpoch, TEST_LOCATION);
...
{
  UiTestApplication application;
  tet_infoline("Every pass ends the epoch, so the NEXT invalidation walks and registers again");
  ...
  for(int round = 0; round < 3; ++round)
  {
    ...
    const uint32_t epochBefore = LIE::CurrentEpoch();
    chain.leaf.InvalidateMeasure();

    DALI_TEST_EQUALS(DataOf(chain.leaf).GetMeasurePropagationEpoch(), epochBefore, TEST_LOCATION);
    DALI_TEST_CHECK(!DataOf(chain.root).IsMeasureCacheValid());

    Settle(application);

    // The drain consumed the registration and started a new epoch.
    DALI_TEST_CHECK(LIE::CurrentEpoch() != epochBefore);
    DALI_TEST_CHECK(DataOf(chain.leaf).GetMeasurePropagationEpoch() != LIE::CurrentEpoch());
  }
...
{
  UiTestApplication application;
  tet_infoline("An arrange propagation must not suppress a later measure walk in the same epoch");
  ...
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetArrangePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);
  DALI_TEST_CHECK(!DataOf(chain.root).IsArrangeCacheValid());
  DALI_TEST_CHECK(DataOf(chain.root).IsMeasureCacheValid());

  // Now measure, in the SAME epoch. A single shared record would make this skip its
  ...
  // computed at all.
  chain.leaf.InvalidateMeasure();
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetMeasurePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);
  DALI_TEST_CHECK(!DataOf(chain.root).IsMeasureCacheValid());
  DALI_TEST_CHECK(DataOf(chain.root).IsMeasureDirty());
...
int UtcDaliLayoutInvalidationNotCoalescedDuringLayoutPassP(void)
{
  ...
  // Step (c) must have landed on mid AFTER step (b) published its entry. If the epoch
  // short-circuit were still live during the pass, step (c) would have returned early
  // -- leaf's record already named the current epoch from step (a) -- and mid would
...
int UtcDaliLayoutInvalidationEpochRetractedByLogicalContextResetP(void)
{
  UiTestApplication application;
  ...
  chain.leaf.InvalidateMeasure();
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetMeasurePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);
  DALI_TEST_EQUALS(DataOf(chain.mid).GetMeasurePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);

  // Any path that can re-root a subtree's context runs through
  // InvalidateLogicalContextRecursive, which is where the records are retracted: a
  ...
  DALI_TEST_EQUALS(DataOf(chain.mid).GetMeasurePropagationEpoch(), 0u, TEST_LOCATION);
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetMeasurePropagationEpoch(), 0u, TEST_LOCATION);
  DALI_TEST_EQUALS(DataOf(chain.leaf).GetArrangePropagationEpoch(), 0u, TEST_LOCATION);
  ...
  DALI_TEST_EQUALS(DataOf(chain.root).GetMeasurePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);
...
int UtcDaliLayoutInvalidationReparentReachesTheNewRootP(void)
{
  ...
  DALI_TEST_EQUALS(DataOf(child).GetMeasurePropagationEpoch(), LIE::CurrentEpoch(), TEST_LOCATION);
...
int UtcDaliLayoutInvalidationManualPassEndsTheEpochP(void)
{
  UiTestApplication application;
  tet_infoline("A manual Measure/Arrange pass ends the epoch, so an invalidation after it walks in full");
  ...
  // still pending, which is exactly what the pass-guard epoch bump exists for: if the
  ...
  // The manual pass ended the epoch...
  DALI_TEST_CHECK(DataOf(chain.leaf).GetMeasurePropagationEpoch() != LIE::CurrentEpoch());
