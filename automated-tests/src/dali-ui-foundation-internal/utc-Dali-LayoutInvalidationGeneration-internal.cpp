...
// DataOf()). The public View::InvalidateMeasure() / View::InvalidateArrange() add a
// once-per-View diagnostic inside Measure/Arrange or LayoutFinished, then execute the
// same complete invalidation transaction. The controller retains the propagated root
// but suppresses an idle wake for work generated by processing itself. DataOf() is
// still used in the staged test to isolate generation/poison mechanics from the
// public diagnostic latch.
...
// (DataOf(view).InvalidateArrange()), not the public View handle method, to exercise
// the generation short-circuit without also setting/testing the public diagnostic
// latch. Both routes now walk, poison and register from inside a pass; wake suppression
...
// --- Warn-latch harness: what the in-pass public invalidation entry points write,
// --- observed from inside the pass that triggers them.
//
...
MeasuredSize WarnLatchMeasure(View, float, float)
{
  ...
  // PUBLIC entry point, from inside the pass: warned once, then fully invalidated.
  ...
  // silently diagnosed -- the invalidation still runs; only the log is latched.
  gWarnLatchView.InvalidateMeasure();
...
// utc-Dali-View.cpp, pinning two internal effects that no public observation can
// reach directly:
//
...
//  2. the complete transaction RAISES the dirty bits rather than merely dropping
//     caches. That raise makes the running pass decline its cache publish and a later
//     cache test miss, while propagation retains the root for later processing.
//
...
{
  UiTestApplication application;
  tet_infoline("The in-pass invalidation warn latch is set once per View, while every call still raises dirty through the full transaction");
  ...
  // Entry consumed it, the in-pass call raised it back: a RAISE, not a leftover.
