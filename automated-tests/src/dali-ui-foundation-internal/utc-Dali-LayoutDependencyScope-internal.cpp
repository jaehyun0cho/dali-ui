...
void AdapterCreateViewHolder(ItemViewHolder& holder)
{
  ++gCreatedItemViews;
  View item = View::New();
...
void AdapterBindViewHolder(ItemViewHolder&)
{
}
...
// The 19 layouter-boundary calls in RecyclerViewImpl are funnelled through
...
int UtcDaliLayoutDependencyRecyclerOwnerIdentityP(void)
{
  ...
  adapter.CreateViewHolderSignal().Connect(&AdapterCreateViewHolder);
  adapter.BindViewHolderSignal().Connect(&AdapterBindViewHolder);
