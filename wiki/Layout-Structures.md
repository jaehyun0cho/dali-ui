...
When attachment presence is not relevant, `GetLayoutParamsOrDefault<T>()`
returns the attached snapshot or the layout manager's default value. The call
does not attach parameters or invalidate layout.

```cpp
auto effective = viewA.GetLayoutParamsOrDefault<GridLayoutParams>();
effective.SetRow(2);              // viewA is unchanged
viewA.SetLayoutParams(effective); // commit and invalidate layout
```
...
- **LayoutParams use value semantics.** `SetLayoutParams()` stores an independent copy. `TryGetLayoutParams(out)` distinguishes attachment presence, while `GetLayoutParamsOrDefault<T>()` returns an attached snapshot or defaults without attaching or invalidating. A modified snapshot must be passed back to `SetLayoutParams()` to commit changes.
