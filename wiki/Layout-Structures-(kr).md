...
기본 View는 측정과 MATCH_PARENT 축 배치에 같은 자연 공간의 자식 예산 계산식을 사용합니다. 배치의 공간은 실제 슬롯에서 구하며 부모의 요청 크기나 min/max를 다시 적용하지 않습니다. 유한한 양수 스케일, 유한한 0 이상 inset, 유한한 중간 계산값일 때 이 식을 공유하고, 그 밖에는 기존 시각 공간의 뺄셈을 유지합니다. WRAP/fixed 축은 측정된 슬롯을 유지합니다. 정규화 차이를 포함하여 유효 입력이 다르면 재측정이 필요하며, 식 공유가 모든 측정·배치 키의 일치를 보장하지는 않습니다.
