...
- hit이 되려면 다음이 모두 성립해야 합니다. 완료된 측정이 게시한 유효한 항목이 있고, view가 measure-dirty가 아니며, pass가 오염되지 않았고, 유효 스케일이 정확히 일치하며, 정규화 및 min/max 적용 후의 두 float 제약 조건도 수치적으로 정확히 같아야 합니다.
- 스케일과 제약 조건은 캐시 KEY의 항입니다. 불일치하면 해당 View의 측정 producer와 필요한 자식 계산이 실행되며 배치 캐시 재사용도 철회될 수 있습니다. 작은 제약 변화도 줄바꿈 경계를 넘을 수 있으므로 키를 허용 오차로 넓히지 않습니다.
...
기본 View는 측정과 MATCH_PARENT 축 배치에 같은 자연 공간의 자식 예산 계산식을 사용합니다. 배치에서는 기본적으로 실제 슬롯을 부모 스케일로 정규화합니다. 유한한 0 이상 요청 크기의 스케일 적용 결과가 유한하고 실제 슬롯과 정확히 같을 때만 요청 자연값을 선택하여 고정 크기 측정의 피연산자를 유지합니다. 외부 슬롯은 바꾸지 않으며 부모 min/max도 다시 적용하지 않습니다. 유한한 양수 스케일, 유한한 0 이상 inset, 유한한 중간 계산값일 때 식을 공유하고, 그 밖에는 기존 시각 공간의 뺄셈을 유지합니다. 부모 입력은 자식 callback 전에 캡처하고 MATCH_PARENT 자식이 있는 축만 content를 계산합니다. WRAP/fixed 자식 축은 측정된 슬롯을 유지하며 MATCH_PARENT 재측정 결과로 어느 배치 축도 교체하지 않습니다. 이 정책은 기본 View에 한정됩니다. manager 예산이나 다른 유효 입력의 차이로 반복 미스가 남을 수 있으며 식 공유가 모든 측정·배치 키의 일치를 보장하지는 않습니다.

producer 입력의 clamp, 자식 예산 선택, 최종 결과의 clamp는 별개 단계입니다.
기본 View는 고정 요청 크기를 자식 예산으로 선택하고, manager 래퍼도 padding을
빼기 전에 스케일을 적용한 고정 요청값을 선택합니다. 이 고정 요청 예산에는 부모
min/max를 적용하지 않으며 각 자식의 bounds는 별도로 적용합니다. 예를 들어 스케일
1, inset 0에서 요청 폭 100, 최대 폭 80인 기본 View는 자식에게 예산 100을 주고
자신의 측정 폭은 80으로 반환합니다. 실제 슬롯이 80이면 MATCH_PARENT 자식을
80으로 재측정하고, 슬롯이 100이면 부모와 자식의 배치는 그 슬롯을 따릅니다.
이는 기존 cold Measure와 실제 슬롯 우선 정책을 유지한 것입니다.

manager 래퍼의 두 단계는 별도 산술을 유지합니다. 부모가 MATCH_PARENT인 축에서
Measure는 정규화 및 min/max 적용 후의 자연 제약에 스케일을 다시 곱하고,
Arrange는 실제 시각 슬롯에서 스케일을 적용한 padding을 뺍니다. 이 왕복과 후속
뺄셈으로 자식의 정규화된 키가 달라질 수 있습니다. padding이 있다는 이유만으로
항상 미스하는 것은 아닙니다.

`UtcDaliLayoutConstraintCacheMatchParentManagerDiagnosticP`의 이전 Debug 실행에서는
warm-up 후 아래 결과가 관찰되었습니다. 조건은 세로 Stack 또는 행·열 각각 하나인
Star Grid, 스케일 `1.1`, 부모·자식 폭 MATCH_PARENT, 고정 자연 높이 `20`, 마진 0,
슬롯 `(0, 0, 922.50830078125, 22)`입니다. 각 행은 명시적으로
`parent.InvalidateMeasure()`만 호출하는 16 pass의 합계이며, 수평 padding은
start와 end에 똑같이 나눠 적용했습니다.

| 수평 padding 합 | 부모 측정/배치 producer | 자식 측정/배치 producer |
|---|---|---|
| 0 | 16/16 | 0/0 |
| 2 | 16/16 | 32/16 |

두 manager에서 관찰한 이 fixture의 실제 producer 실행 수이며, Measure API 호출
수나 프레임 시간이 아닙니다. 자식 횟수는 출력만 하며 assertion으로 검사하지
않습니다. manager로 예산 공유를 확장하려면 실제 사용 조건의 계측과 함께 content
bounds, fixed/WRAP 슬롯, min/max 처리, Grid track과 Flex 줄바꿈 경계의 배치
결과를 검증해야 합니다.
...
#### 8.3.6 Recycler extent 저장

`LinearItemsLayouterImpl`은 확장이 필요할 때 필요한 prefix 이상을 확보하고,
그 밖에는 extent 캐시 용량을 기존의 약 1.5배로 늘립니다. 상한은 현재 항목 수와
필요한 prefix 중 큰 값이며 저장 한도도 적용합니다. 이는 미리 알려진 고정 항목
집합을 순차 측정할 때 반복되는 prefix 복사를 줄입니다. cold sparse 측정은
전체 목록이 아니라 요청 위치까지의 prefix만 예약합니다.

항목 범위 안의 위치를 위해 확장할 때는 현재 항목 수를 넘는 여유를 두지
않습니다. 다만 이전에 할당한 용량은 항목 수가 줄어도 유지합니다. 항목을 하나씩
추가하고 매번 새 끝까지 캐시를 다시 채워 `Size == Capacity == count`에
도달한다면, 확장할 때마다 이전 prefix를 복사하므로 누적 복사 원소 수는
Theta(n^2)로 남을 수 있습니다. 가상화된 목록의 모든 append가 이 조건을
만족하는 것은 아닙니다. 어댑터 변경은 캐시 size를 비우지만 capacity는 유지합니다.

View 필드를 추가하지 않아도 벡터 여유 용량과 재할당 중 구·신 버퍼가 겹치는
기간의 heap 비용은 발생합니다. storage UTC는 기존 인터페이스를 통해 geometry,
range와 reset 동작을 검사하며 capacity·할당 횟수·복사 원소 수는 계측하지
않습니다. 성장 정책을 바꾸려면 복사량과 peak heap을 측정하여 메모리 비용을
결정해야 합니다.
