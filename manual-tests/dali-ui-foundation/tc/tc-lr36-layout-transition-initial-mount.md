# LR36. Layout: Transition initial mount

이 문서는 [tc-lr36-layout-transition-initial-mount.cpp](tc-lr36-layout-transition-initial-mount.cpp)의 신규 TC만 실행하는 절차입니다. 이전 Layout TC의 실행 결과를 이 TC의 증거로 합산하지 마십시오.
...
2. 검증 artifact의 `manual-tests/dali-ui-foundation/bin/manual-test-dali-ui-foundation`을 실행하고 stdout을 손실 없이 저장하십시오. launcher에서 `LR36. Layout: Transition initial mount`를 선택하십시오. 격리 build인 경우 해당 artifact 안의 binary를 실행하십시오.
3. `LR36.Reset run`을 한 번 눌러 새 run ID를 기록하십시오. 이후 실패를 지우기 위해 Reset하지 마십시오. 버튼 표시는 `Reset run`이며 Actor name과 accessibility name은 앞의 TC ID를 포함합니다.
4. 각 scenario에서 `LR36.Run scenario`를 누르면 남은 step이 순서대로 자동 실행됩니다(step 하나씩 진행하려면 `LR36.Run step`). `LR_ACTION`의 scenario/step/required_checks가 아래 순서와 일치하는지 먼저 확인하십시오. 각 step이 끝나면 `LR_READY`와 그 step의 모든 행이 자동 출력됩니다.
...
6. 각 step의 모든 검사 행은 완료 시 stdout에 자동 출력되며 HUD에는 요약과 실패 행만 표시되므로 행 수와 숫자는 stdout에서 빠짐없이 수집하십시오. 모든 step이 끝난 뒤에만 `LR36.Next scenario`를 누르거나, `LR36.Run all`로 남은 scenario를 이어서 자동 실행하십시오. 총 scenario 수는 5개입니다. `Run all` 뒤에는 마지막 `LR_RESULT`의 `completed_scenarios`가 `required_scenarios`와 같아질 때까지 기다리십시오. 그 전의 `FAILING`/`INCOMPLETE`는 종결 verdict가 아니므로 실행을 중단하지 마십시오. 완료 후 `< Back`으로 나가 `LR_CASE_END`의 `completed_scenarios`가 `required_scenarios`와 같은지 확인하십시오.

## 단계와 필수 관측 수

`TR02.mount-0`부터 `mount-3`까지 각각 `mount` 4개 → `settled` 7개 → `runtime-add` 7개입니다. 마지막 `TR02.attach-after-add`는 `mount` 4개 → `attach-and-invalidate` 6개입니다.
...
- mode 0/1은 Spec, 2/3은 Animator입니다. 짝수는 초기 ENTER 억제, 홀수는 초기 ENTER 허용입니다. 초기 target/rendered bounds는 (40,30,80,40)입니다.

- `settled`는 700ms 뒤 관측합니다. 초기 start/finish는 짝수 0/0, 홀수 1/1입니다. Spec은 초기 opacity 0을 최종 1로 적용하고 Animator observer는 opacity를 쓰지 않으므로 0을 유지합니다.

- `runtime-add`는 (140,30,60,20)의 sibling을 새로 추가하고 700ms 뒤 완료를 검사합니다. 처음부터 붙어 있던 child에 뒤늦게 transition을 붙이는 마지막 scenario는 ENTER/finish 0을 유지해야 합니다.
...
Profile `diagnostic / release`: 5 scenario, 14 action, 82 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `TR02.mount-0` | `mount` : 4 → `settled` : 7 → `runtime-add` : 7 |
| `TR02.mount-1` | `mount` : 4 → `settled` : 7 → `runtime-add` : 7 |
| `TR02.mount-2` | `mount` : 4 → `settled` : 7 → `runtime-add` : 7 |
| `TR02.mount-3` | `mount` : 4 → `settled` : 7 → `runtime-add` : 7 |
| `TR02.attach-after-add` | `mount` : 4 → `attach-and-invalidate` : 6 |
