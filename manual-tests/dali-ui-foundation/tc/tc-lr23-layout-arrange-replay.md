...
- `K08.actor-repair`: 첫 target(7,9,37,19) 이후 Actor position/size를 직접 손상시킵니다. 같은 Arrange가 producer를 추가 실행하지 않고 target을 복원해야 합니다. PARENT_ORIGIN.x=0, PIVOT.x=0.5, POSITION_USES_PIVOT=false여야 합니다. transform 중심인 PIVOT과 layout의 top-left 배치를 구분하십시오. 이 scenario의 필수 검사는12행입니다.
- `K09.subtree-replay`: parent cache hit에서도 child b의 외부 position 손상을 복원해야 합니다. child producer 횟수는 그대로입니다.
- `K10.returned-bounds`: Measure(100,50)를 먼저 수행하여 유효한 measure cache를 만든 뒤 입력(0,0,100,50)에 callback이 반환한(11,13,53,29)을 검증합니다. 최초와 hit 모두 반환 rect가 같고 producer는1회여야 합니다.
...
Profile `diagnostic / release`: 3 scenario, 3 action, 31 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K08.actor-repair` | `run` : 12 |
| `K09.subtree-replay` | `run` : 10 |
| `K10.returned-bounds` | `run` : 9 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR23 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
