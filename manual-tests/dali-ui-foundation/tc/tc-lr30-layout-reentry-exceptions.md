# LR30. Layout: Reentry and exceptions

이 TC는 신규 Layout suite의 **Reentry and exceptions** 검증입니다. 대응 소스는 `tc-lr30-layout-reentry-exceptions.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR30`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `K18.measure-throw`: 최초 37×19 이후 producer 예외를 주입합니다. 예외는 호출자에게 전달되고 마지막 완료 결과 37×19는 유지되어야 합니다. 예외 제거 후 53×19를 계산합니다.
- `K18.arrange-throw`: Measure(100,50)와 완료된 Arrange(1,2,37,19) 이후 Arrange(7,9,53,29)의 producer가 예외를 던집니다. ArrangeImpl은 callback 전에 input을 Actor에 provisional geometry로 적용하므로 provisional-input은(7,9,53,29)입니다. 마지막 완료 record는 별도이며 ViewImpl::GetArrangedBounds()로 읽은 last-completed는(1,2,37,19)를 유지해야 합니다. 예외를 제거한 retry는(7,9,53,29), producer 누적 3이어야 합니다. 같은 입력 retry-cache는 같은 rect이고 producer가 3에 머물러야 합니다. 이 scenario는 19행이며 Actor의 provisional 상태와 완료 publication을 혼동하지 마십시오.
- `K18.measure-reentry`: callback 안 동일 View Measure는 무한 재귀하지 않아야 합니다. Debug 계약 예외 또는 Release guard 후 다음 정상 호출 37×19가 성공해야 합니다. 예외 유무만으로 PASS를 결정하지 않습니다.

- 예외는 `LayoutController::ProcessLayouts()`(동기 controller pass)를 통해 호출자에게 전파됩니다. `K18.measure-throw`: `mount` → `throw`(throwing producer + InvalidateMeasure + ProcessLayouts: 예외 전파, 마지막 완료 measured 37×19, 렌더 불변) → `retry`(producer 정상화, 53×19 렌더). `K18.arrange-throw`: `mount`((1,2,37,19)) → `throw`(입력 (7,9,53,29)는 provisional Actor geometry, 완료 기록은 (1,2,37,19)) → `retry`(렌더 (7,9,53,29), producer +2) → `cache`(형제 pass에서 producer 불변). `K18.measure-reentry`: `mount` → `reenter`(callback 안 재진입 Measure, bounded) → `recover`(37×19).
...
exception 발생 시 완료 결과 publication을 보존하고 재시도할 수 있는지, reentry guard가 복구되는지 검사합니다. view-data-impl.cpp의 ArrangePassGuard는 cache/in-progress 상태를 정리하며, ArrangeImpl의 Phase 1 Actor 적용은 callback보다 먼저 실행됩니다. 이 검사는 예외 발생 시 모든 Actor property가 이전 값으로 rollback된다는 계약을 가정하지 않습니다. Debug의 DaliException은 좁은 scope에서 잡으며 프로그램 종료를 요구하지 않습니다.
...
Profile `diagnostic / release`: 3 scenario, 10 action, 52 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K18.measure-throw` | `mount` : 4 → `throw` : 7 → `retry` : 6 |
| `K18.arrange-throw` | `mount` : 4 → `throw` : 9 → `retry` : 5 → `cache` : 5 |
| `K18.measure-reentry` | `mount` : 4 → `reenter` : 2 → `recover` : 6 |
