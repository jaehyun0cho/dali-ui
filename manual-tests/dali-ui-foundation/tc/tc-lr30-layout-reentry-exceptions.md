...
- `K18.measure-throw`: 최초37×19 이후 producer 예외를 주입합니다. 예외는 호출자에게 전달되고 마지막 완료 결과37×19는 유지되어야 합니다. 예외 제거 후53×19를 계산합니다.
- `K18.arrange-throw`: Measure(100,50)와 완료된 Arrange(1,2,37,19) 이후 Arrange(7,9,53,29)의 producer가 예외를 던집니다. ArrangeImpl은 callback 전에 input을 Actor에 provisional geometry로 적용하므로 provisional-input은(7,9,53,29)입니다. 마지막 완료 record는 별도이며 ViewImpl::GetArrangedBounds()로 읽은 last-completed는(1,2,37,19)를 유지해야 합니다. 예외를 제거한 retry는(7,9,53,29), producer 누적3이어야 합니다. 같은 입력 retry-cache는 같은 rect이고 producer가3에 머물러야 합니다. 이 scenario는19행이며 Actor의 provisional 상태와 완료 publication을 혼동하지 마십시오.
- `K18.measure-reentry`: callback 안 동일 View Measure는 무한 재귀하지 않아야 합니다. Debug 계약 예외 또는 Release guard 후 다음 정상 호출37×19가 성공해야 합니다. 예외 유무만으로 PASS를 결정하지 않습니다.
...
Profile `diagnostic / release`: 3 scenario, 3 action, 28 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K18.measure-throw` | `run` : 5 |
| `K18.arrange-throw` | `run` : 19 |
| `K18.measure-reentry` | `run` : 4 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR30 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
