# LR02. Layout: Core invalid inputs

이 TC는 신규 Layout suite의 **Core invalid inputs** 검증입니다. 대응 소스는 `tc-lr02-layout-core-invalid-inputs.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR02`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `C09.reject.0.3`: -3/NaN/+Inf/-Inf 설정 후 requested=37×19, min=3×2, max=100×80을 그대로 유지해야 합니다.
- `C09.max-wins`: min이 max보다 커도 측정은 max인 40×30입니다.
- `C09.sentinel`: -1/-2의 ±0.0005는 정확한 sentinel로 정규화됩니다.
- `R03.negative-flex-factors`: grow=-1/shrink=-2는 각각 0입니다.

- `C09.reject.{i}`는 getter 6행에 더해 200×100 stage 부착 후 measured 37×19와 렌더 (0,0,37,19)를 검사합니다(12행). `C09.max-wins.*`는 300×300 stage에서 measured 40×30과 렌더를, `C09.sentinel.*`는 canonical getter 2행에 더해 MATCH_PARENT는 (0,0,200,100), WRAP_CONTENT는 (0,0,0,0)의 렌더 4행을 검사합니다.
- `R03.returned-rect.{i}`는 `invalid-result`(53×29 probe를 margin (7,0,9,0)으로 부착한 뒤 callback이 무효 rect를 반환하는 상태에서 **동기** `ProcessLayouts()` 한 번을 try/catch 안에서 수행: Debug library는 `DALI_ASSERT_DEBUG`의 DaliException을 던지고 그 예외는 이 step이 포착하며, Release library는 입력 rect로 fallback합니다. 두 경우 모두 `invalid-result-contained`=true(예외 포착 또는 event-side (7,9,53,29))이고, 훅을 제거한 뒤 재arrange한 렌더 결과는 (7,9,53,29)입니다) → `recovered`(callback 정상화 후 61×31, margin (11,0,13,0): 렌더 (11,13,61,31))의 두 step입니다.
...
- `R03.returned-rect.0.2`: custom callback의 음수 extent/NaN position/Inf extent를 반환합니다. Debug는 DaliException(step 안의 동기 pass에서 포착), Release는 전체 input(7,9,53,29) fallback이어야 하며, 어느 쪽이든 앱이 abort되면 FAIL입니다. callback 전에 유효 input이 Actor에 provisional geometry로 적용되므로 safe-actor는 두 경우 모두(7,9,53,29)입니다. 잘못된 반환의 부분 field만 채택해서는 안 됩니다. 완료 record publication의 보존은 LR30에서 ViewImpl::GetArrangedBounds()로 별도 검사합니다. 다음 정상 호출(11,13,61,31)로 복구해야 합니다.

## 근거와 검증 경계

`IsValidRequestedSize`, `IsValidSizeBound`, max-wins 공개 계약과 Flex factor normalization을 검사합니다. NaN/Inf를 일반 geometry의 정상 expected로 승인하지 않습니다.
...
Profile `diagnostic / release`: 13 scenario, 16 action, 107 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `C09.reject.2` | `run` : 12 |
| `C09.reject.3` | `run` : 12 |
| `C09.max-wins.20.000000` | `run` : 6 |
| `C09.max-wins.70.000000` | `run` : 6 |
| `C09.max-wins.150.000000` | `run` : 6 |
| `C09.sentinel.-1.000000` | `run` : 6 |
| `C09.sentinel.-2.000000` | `run` : 6 |
| `R03.negative-flex-factors` | `run` : 2 |
| `R03.returned-rect.0` | `invalid-result` : 5 → `recovered` : 4 |
| `R03.returned-rect.1` | `invalid-result` : 5 → `recovered` : 4 |
| `R03.returned-rect.2` | `invalid-result` : 5 → `recovered` : 4 |
