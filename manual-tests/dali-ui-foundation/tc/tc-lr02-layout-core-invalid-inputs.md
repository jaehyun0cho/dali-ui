...
- `R03.returned-rect.0..2`: custom callback의 음수 extent/NaN position/Inf extent를 반환합니다. Debug는 DaliException, Release는 전체 input(7,9,53,29) fallback이어야 합니다. callback 전에 유효 input이 Actor에 provisional geometry로 적용되므로 safe-actor는 두 경우 모두(7,9,53,29)입니다. 잘못된 반환의 부분 field만 채택해서는 안 됩니다. 완료 record publication의 보존은 LR30에서 ViewImpl::GetArrangedBounds()로 별도 검사합니다. 다음 정상 호출(11,13,61,31)로 복구해야 합니다.
...
Profile `diagnostic / release`: 13 scenario, 13 action, 71 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
...
| `C09.reject..2` | `run` : 8 |
| `C09.reject..3` | `run` : 8 |
| `C09.max-wins.20.000000` | `run` : 2 |
| `C09.max-wins.70.000000` | `run` : 2 |
| `C09.max-wins.150.000000` | `run` : 2 |
| `C09.sentinel.-1.000000` | `run` : 2 |
| `C09.sentinel.-2.000000` | `run` : 2 |
| `R03.negative-flex-factors` | `run` : 2 |
| `R03.returned-rect..0` | `run` : 9 |
| `R03.returned-rect..1` | `run` : 9 |
| `R03.returned-rect..2` | `run` : 9 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR02 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
