...
4. 각 scenario에서 `LR35.Run scenario`를 누르면 남은 step이 순서대로 자동 실행됩니다(step 하나씩 진행하려면 `LR35.Run step`). `LR_ACTION`의 scenario/step/required_checks가 아래 순서와 일치하는지 먼저 확인하십시오. 각 step이 끝나면 `LR_READY`와 그 step의 모든 행이 자동 출력됩니다.
...
6. 각 step의 모든 검사 행은 완료 시 stdout에 자동 출력되며 HUD에는 요약과 실패 행만 표시되므로 행 수와 숫자는 stdout에서 빠짐없이 수집하십시오. 모든 step이 끝난 뒤에만 `LR35.Next scenario`를 누르거나, `LR35.Run all`로 남은 scenario를 이어서 자동 실행하십시오. 총 scenario 수는 4개입니다. `Run all` 뒤에는 마지막 `LR_RESULT`의 `completed_scenarios`가 `required_scenarios`와 같아질 때까지 기다리십시오. 그 전의 `FAILING`/`INCOMPLETE`는 종결 verdict가 아니므로 실행을 중단하지 마십시오. 완료 후 `< Back`으로 나가 `LR_CASE_END`의 `completed_scenarios`가 `required_scenarios`와 같은지 확인하십시오.

## 단계와 필수 관측 수

`TR01.defaults-values`: `run` 20개 → `TR13.registration-rejection`: `run` 33개 → `TR05.factory-values`: `run` 64개 → `TR13.nonfinite-robustness`: `run` 20개입니다.
...
- nonfinite-robustness는 NaN/+Inf의 size X/Y, anchor X/Y, offset X, bounds duration과 CHANGE timing duration/delay, animator timing duration/delay를 각각 검사하는 robustness 요구입니다. setter가 값을 수용하면 해당 행은 FAIL입니다. 공개 문서의 명시 계약과 robustness 요구를 혼동하지 마십시오. 현재 구현의 수용 결과를 golden으로 바꾸거나 이 20행을 제외하여 PASS로 처리하지 마십시오.
...
- 비유한 입력: `robustness.nonfinite.{nan,positive-infinity}.{size-x,size-y,anchor-x,anchor-y,offset-x,duration,change-duration,change-delay,animator-duration,animator-delay}.rejected`입니다. 20개 ID가 모두 있어야 합니다. 실패하면 어떤 입력이 수용되었는지 이 ID로 분리하십시오.
...
Profile `diagnostic / release`: 4 scenario, 4 action, 139 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `TR01.defaults-values` | `run` : 20 |
| `TR13.registration-rejection` | `run` : 35 |
| `TR05.factory-values` | `run` : 64 |
| `TR13.nonfinite-robustness` | `run` : 20 |
