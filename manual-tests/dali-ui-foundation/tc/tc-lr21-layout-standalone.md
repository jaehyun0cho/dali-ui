...
- `S10.standalone.0..4`: default/Stack/Flex/Grid/Absolute에 ordinary20×10과 standalone100×80을 추가합니다. parent intrinsic은20×10입니다. standalone의 SetRequestedX/Y를7/9로 설정한 뒤 임시로(77,99,100,80)에 직접 Arrange합니다. parent Arrange는 이전 Actor 좌표를 보존하는 대신 요청에서 도출한(7,9,100,80)으로 복원해야 합니다. 논리 child 수는2입니다.
- `G14.mode-change`: Stack20+gap7+30=57×15에서 둘째를 STANDALONE으로 바꾸면20×10입니다.


- 추가 S10.match-margin-clamp.0..4를 default/Stack/Flex/Grid/Absolute 각각 실행하십시오. 각8행입니다. parent200×100의 padding(start20,end30,top40,bottom10), standalone MATCH_PARENT 양축, 자신의 margin(7,11,13,17), 요청 위치(5,9)를 사용합니다.
- 첫 expected rect는(5+7,9+13,200−7−11,100−13−17)=(12,22,182,70)입니다. parent padding을 extent나 위치에서 다시 빼면 FAIL입니다.
- 이어 minWidth190/maxWidth195/minHeight20/maxHeight60을 설정하면 width=min(max(182,190),195)=190, height=min(max(70,20),60)=60이며 위치는(12,22)입니다. width가 가용182보다 커지는 것은 명시적인 minimum의 결과이며 임의로182에 제한하지 마십시오.
...
Profile `diagnostic / release`: 11 scenario, 11 action, 79 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `S10.standalone..0` | `run` : 7 |
| `S10.standalone..1` | `run` : 7 |
| `S10.standalone..2` | `run` : 7 |
| `S10.standalone..3` | `run` : 7 |
| `S10.standalone..4` | `run` : 7 |
| `G14.mode-change` | `run` : 4 |
| `S10.match-margin-clamp..0` | `run` : 8 |
| `S10.match-margin-clamp..1` | `run` : 8 |
| `S10.match-margin-clamp..2` | `run` : 8 |
| `S10.match-margin-clamp..3` | `run` : 8 |
| `S10.match-margin-clamp..4` | `run` : 8 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR21 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
