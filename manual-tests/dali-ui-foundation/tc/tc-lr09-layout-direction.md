# LR09. Layout: Direction inheritance

이 TC는 신규 Layout suite의 **Direction inheritance** 검증입니다. 대응 소스는 `tc-lr09-layout-direction.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR09`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- C15.LTR-RTL-LTR: A/B의 LTR x는 0/27, RTL x는 200−0−20=180 및 200−27−30=143입니다. 원래 방향 복원 후 최초 값과 같아야 합니다.
- explicit-child: parent RTL이 child를 x=200에 배치하지만 child의 명시 LTR 내부 leaf는 x=0입니다.

- `C15.LTR-RTL-LTR`은 `ltr.0` → `rtl.1` → `ltr.2`의 세 step입니다. RTL step은 렌더 rect(a (180,0,20,10), b (143,0,30,15))에 더해 controller가 View LayoutFinished로 보고한 최종(post-RTL) bounds(`a.reported`=(180,0,20,10), `b.reported`=(143,0,30,15)) 8행을 추가로 검사합니다(16행). 보고 값이 pre-RTL logical rect(0/27)이면 FAIL입니다.
...
- `C15.standalone-replay.{kind}`(0 View, 1 Stack, 2 Flex, 3 Grid, 4 Absolute): 200×60 container에 a 20×10, b 30×15와 STANDALONE 10×10(요청 (150,40))을 두고 `ltr` → `rtl`(a/b는 x'=200−x−w로 mirror, standalone은 (150,40) 유지) → `replay`(RTL 상태에서 container `InvalidateArrange()`: 입력이 같으므로 자식은 cache-hit replay 경로를 지나며 mirror가 접힌 채 유지되어야 하고 이중 mirror나 복원은 FAIL) → `ltr-again`(복원)의 4 step, step마다 12행입니다. LTR 위치는 View/Absolute a (5,7)·b (40,20), Stack/Flex a (0,0)·b (20,0), Grid(암시 cell, START 정렬) a·b 모두 (0,0)입니다.

## 근거와 검증 경계

부모가 적용하는 direct-child mirror와 자식 subtree의 effective direction을 구분합니다. logical Arrange 반환을 mirrored Actor x와 무조건 동일시하지 않습니다.
...
Profile `diagnostic / release`: 7 scenario, 24 action, 280 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C15.LTR-RTL-LTR` | `ltr.0` : 8 → `rtl.1` : 16 → `ltr.2` : 8 |
| `C15.explicit-child` | `run` : 8 |
| `C15.standalone-replay.0` | `ltr` : 12 → `rtl` : 12 → `replay` : 12 → `ltr-again` : 12 |
| `C15.standalone-replay.1` | `ltr` : 12 → `rtl` : 12 → `replay` : 12 → `ltr-again` : 12 |
| `C15.standalone-replay.2` | `ltr` : 12 → `rtl` : 12 → `replay` : 12 → `ltr-again` : 12 |
| `C15.standalone-replay.3` | `ltr` : 12 → `rtl` : 12 → `replay` : 12 → `ltr-again` : 12 |
| `C15.standalone-replay.4` | `ltr` : 12 → `rtl` : 12 → `replay` : 12 → `ltr-again` : 12 |
