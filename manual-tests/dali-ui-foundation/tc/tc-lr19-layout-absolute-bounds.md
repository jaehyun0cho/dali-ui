# LR19. Layout: Absolute bounds

이 TC는 신규 Layout suite의 **Absolute bounds** 검증입니다. 대응 소스는 `tc-lr19-layout-absolute-bounds.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR19`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `A01.explicit-local`: parent를(70,90)에서(100,120)으로 이동해도 child는 parent-local(11,13,37,19)입니다.
- `A02.extent.*`: x−20, y7, height19에 width0/1/37/250을 그대로 배정합니다. zero와 음수 위치, 유효 overflow를 구분하십시오.
- `A03.intrinsic-match`: bounds extent−1은 child requested extent37×19에 위임합니다. MATCH_PARENT로 바꾼 후 100×60입니다.
- `A07.wrap-negative-position`: far edge(−20+100,−5+20)=(80,15)입니다.

- `A01.explicit-local`은 `mount`(layout을 margin (70,0,90,0)으로 300×220 stage에 부착: parent (70,90,200,100), child (11,13,37,19)) → `move-parent`(margin (100,0,120,0): parent 이동, child 불변)이고, `A03.intrinsic-match`는 `intrinsic`((5,7,37,19)) → `match`(MATCH_PARENT: (5,7,100,60))입니다. `A07.wrap-negative-position`은 measured (80,15)와 렌더 (0,0,80,15)를 검사합니다.
...
Absolute의 명시 bounds, 위임 sentinel, 음수 위치와 WRAP far edge를 검사합니다.
...
Profile `diagnostic / release`: 7 scenario, 9 action, 46 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `A01.explicit-local` | `mount` : 8 → `move-parent` : 8 |
| `A02.extent.0` | `run` : 4 |
| `A02.extent.1` | `run` : 4 |
| `A02.extent.37` | `run` : 4 |
| `A02.extent.250` | `run` : 4 |
| `A03.intrinsic-match` | `intrinsic` : 4 → `match` : 4 |
| `A07.wrap-negative-position` | `run` : 6 |
