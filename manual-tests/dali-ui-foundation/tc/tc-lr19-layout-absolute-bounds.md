...
- `A03.intrinsic-match`: bounds extent−1은 child requested extent37×19에 위임합니다. MATCH_PARENT로 바꾼 후100×60입니다.
...
Profile `diagnostic / release`: 7 scenario, 7 action, 34 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `A01.explicit-local` | `run` : 8 |
| `A02.extent.0` | `run` : 4 |
| `A02.extent.1` | `run` : 4 |
| `A02.extent.37` | `run` : 4 |
| `A02.extent.250` | `run` : 4 |
| `A03.intrinsic-match` | `run` : 8 |
| `A07.wrap-negative-position` | `run` : 2 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR19 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
