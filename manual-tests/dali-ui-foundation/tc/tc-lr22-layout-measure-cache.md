# LR22. Layout: Measure cache

이 TC는 신규 Layout suite의 **Measure cache** 검증입니다. 대응 소스는 `tc-lr22-layout-measure-cache.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR22`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `K01.same-key`: 100×50 stage의 첫 pass(`cold`)에서 synthetic producer는 1회이고 렌더 크기는 37×19입니다. 형제 leaf만 바꾼 pass(`hit`)는 횟수 1을 유지하고, stage 높이를 51로 바꾸면(`height-key`) key가 달라져 2회입니다.
- `K02.normalized-key`: minimum width 120와 maximum height 30 때문에 40×100 stage의 raw 제약은 (120,30)으로 정규화됩니다. producer 입력 120/30, measured 120×19, 렌더 (0,0,120,19)를 확인하고, stage를 50×200으로 바꿔도(`resize`) producer는 1회여야 합니다.
- `K03.explicit-invalidation`: 외부 width를 37→53으로 바꾸기만 하면 형제 pass 뒤에도 cached 37입니다. `InvalidateMeasure` 이후 53×19가 측정·렌더되고(producer 2), 다음 형제 pass는 hit입니다.
- `K04.sibling-isolation`: 20+30 stack(50×10) 부착 후 a를 40으로 바꾸고 invalidate하면 root는 70×10이 되고 a producer만 +1, b producer는 그대로입니다.
...
cache key와 normalization, external state invalidation, clean sibling 재사용을 실제 synthetic producer counter로 검증합니다.
...
Profile `diagnostic / release`: 4 scenario, 11 action, 48 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K01.same-key` | `cold` : 7 → `hit` : 3 → `height-key` : 1 |
| `K02.normalized-key` | `mount` : 8 → `resize` : 1 |
| `K03.explicit-invalidation` | `mount` : 5 → `stale` : 3 → `invalidated` : 7 → `fresh-cache` : 1 |
| `K04.sibling-isolation` | `mount` : 4 → `dirty-a` : 8 |
