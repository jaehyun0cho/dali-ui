...
- `K01.same-key`: 같은(100,50)을 두 번 Measure하면 synthetic producer1회, height51로 바꾸면2회입니다.
- `K02.normalized-key`: minimum width120와 maximum height30 때문에 raw(40,100)/(50,200)은 모두(120,30)입니다. producer1회와 measured120×19를 확인하십시오.
- `K03.explicit-invalidation`: 외부 width를37→53으로 바꾸기만 하면 cached37입니다. InvalidateMeasure 이후53이며 다음 동일 입력은 hit입니다.
- `K04.sibling-isolation`: a20→40, b30으로 바꾼 후 parent70입니다. a만 재측정하고 b producer는 증가하지 않아야 합니다.
...
Profile `diagnostic / release`: 4 scenario, 4 action, 22 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K01.same-key` | `run` : 6 |
| `K02.normalized-key` | `run` : 5 |
| `K03.explicit-invalidation` | `run` : 7 |
| `K04.sibling-isolation` | `run` : 4 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR22 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
