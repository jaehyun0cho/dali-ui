...
- `ORDER.non-view`: a와 b 사이 장식 Actor를 integration API로 추가합니다. Actor child 수3, View child 수2이며 b x20입니다.


- 추가 ORDER.preserve-raise / preserve-lower / preserve-lower-below는 각각20행입니다. 초기 Actor와 논리 순서 a,b,c, 각 width20/30/40에서 실행합니다. Raise/Lower는 한 단계, LowerBelow는 지정 sibling 바로 아래라는 공개 동작을 사용합니다.
...
각 scenario는 GetChildAt의 실제 Actor 순서3행과 GetChildViewAt의 논리 순서3행, 두 child count=3의2행을 별도로 검사합니다. 이후 실제 rect는 a=(0,0,20,10), b=(20,0,30,10), c=(50,0,40,10)의12행이어야 합니다. 논리 순서만 맞고 시각 순서가 바뀌지 않는 경우도 FAIL입니다. fresh fixture를 사용하므로 이전 시나리오의 z-order를 expected에 섞지 마십시오.
...
Profile `diagnostic / release`: 6 scenario, 6 action, 87 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `ORDER.update` | `run` : 10 |
| `ORDER.preserve` | `run` : 10 |
| `ORDER.non-view` | `run` : 7 |
| `ORDER.preserve-raise` | `run` : 20 |
| `ORDER.preserve-lower` | `run` : 20 |
| `ORDER.preserve-lower-below` | `run` : 20 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR32 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
