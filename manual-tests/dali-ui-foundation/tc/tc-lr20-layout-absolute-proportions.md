...
- `A08.position.*`: parent300, child100이므로 x=(300−100)×p=200p입니다. p−.25/0/.5/1/1.25는 유효 입력이며 RTL은 200−x입니다.
- `A05.all-flags`: ALL은 모든 현재 proportional bit를 적용하여(50,25,100,50)입니다.

- `A08.position.{p}`는 `ltr`(렌더 (200p,0,100,20)) → `rtl`(RIGHT_TO_LEFT: controller가 보고한 `rtl.reported`와 렌더 `rtl` 모두 (200−200p,0,100,20); 8행)의 두 step입니다. `A06.wrap-circularity`는 `position-only`(40×20) → `size-circular`(0×20)이며 각 step은 measured 2행 + 렌더 4행입니다.
...
- `A06.wrap-circularity`: position만 proportional이면 determinate width40을 WRAP에 기여합니다. width 자체를 parent의.5로 바꾸면 순환하는 width 기여만 제외하여 0×20입니다.
...
Profile `diagnostic / release`: 23 scenario, 29 action, 140 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `A04.flags.14` | `run` : 4 |
| `A04.flags.15` | `run` : 4 |
| `A08.position.-0.250000` | `ltr` : 4 → `rtl` : 8 |
| `A08.position.0.000000` | `ltr` : 4 → `rtl` : 8 |
| `A08.position.0.500000` | `ltr` : 4 → `rtl` : 8 |
| `A08.position.1.000000` | `ltr` : 4 → `rtl` : 8 |
| `A08.position.1.250000` | `ltr` : 4 → `rtl` : 8 |
| `A05.all-flags` | `run` : 4 |
| `A06.wrap-circularity` | `position-only` : 6 → `size-circular` : 6 |
