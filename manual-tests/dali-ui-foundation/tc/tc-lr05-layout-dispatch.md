...
- C17: 같은 View의 기본 bounding 30×15를 먼저 측정한 뒤 horizontal Stack을 late attach하고 callback을 등록하여7×9 우선 결과를 확인하십시오. callback 제거 후에는20+7+30=57, max(10,15)=15여야 합니다. 필수6행은 default-before-attach, callback-priority, manager-after-remove의 width/height입니다. public AttachLayoutManager는 View 생애 중 한 번의 non-null attach만 허용하며 detach API가 아닙니다.
...
Profile `diagnostic / release`: 5 scenario, 5 action, 38 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C17.callback-manager-default` | `run` : 6 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR05 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
