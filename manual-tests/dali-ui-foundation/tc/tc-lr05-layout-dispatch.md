# LR05. Layout: Layout dispatch

이 TC는 신규 Layout suite의 **Layout dispatch** 검증입니다. 대응 소스는 `tc-lr05-layout-dispatch.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR05`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- C17: 같은 View의 기본 bounding 30×15를 먼저 측정한 뒤 horizontal Stack을 late attach하고 callback을 등록하여 7×9 우선 결과를 확인하십시오. callback 제거 후에는 20+7+30=57, max(10,15)=15여야 합니다. 필수 6행은 default-before-attach, callback-priority, manager-after-remove의 width/height입니다. public AttachLayoutManager는 View 생애 중 한 번의 non-null attach만 허용하며 detach API가 아닙니다.
- C18: 같은 입력을 Layout wrapper와 일반 View+manager에 적용하십시오. Stack/Absolute child는 (0,0,20,10), Flex STRETCH는 (0,0,20,100), Grid FILL은 (0,0,200,100)입니다.

- `C17.callback-manager-default`는 `default`(200×100 stage: 30×15) → `callback`(Stack manager 부착 + measure callback: 7×9) → `manager`(callback 제거: 57×15)의 세 step이며 각 step은 measured 2행 + 렌더 4행입니다. `C18.attached-manager.{kind}`는 container와 attached View를 같은 200×220 stage(두 번째는 y=110)에 부착하고 두 child의 렌더 rect를 검사합니다.
...
- `C19.custom-manager`: 앱이 정의한 `LayoutManager` 서브클래스(각 자식을 직전 자식의 우하단에서 `step`배만큼 이동해 대각선 배치)를 plain View에 attach합니다. `mount`(a 20×10, b 30×15, c 40×20: measured 90×45, a (0,0), b (20,10), c (50,25)) → `step-1.5`(manager 소유 상태 setter가 `InvalidateOwnerMeasure()`로 pass를 스스로 예약해야 하며 Measure/Arrange 호출 각 +1, b (30,15), c (75,37.5)) → `same-value`(같은 값을 다시 쓰면 pass가 없어야 함: 300ms 뒤 +0/+0, c 불변) → `sibling-pass`(형제 leaf만 바꾼 pass에서 custom manager는 measure 캐시와 기본 IF_CHANGED 정책으로 재실행되지 않음: +0/+0) → `step-1.0`(복원)의 5 step입니다. 호출 수 delta는 정확히 일치해야 합니다.

## 근거와 검증 경계

callback→attached manager→default의 dispatch 순서와 네 manager의 attachment 경로를 검사합니다. 계측을 위해 built-in manager에 callback을 덮어씌우지 않습니다.
...
Profile `diagnostic / release`: 6 scenario, 12 action, 106 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C17.callback-manager-default` | `default` : 6 → `callback` : 6 → `manager` : 6 |
| `C18.attached-manager.1` | `run` : 8 |
| `C18.attached-manager.2` | `run` : 8 |
| `C18.attached-manager.3` | `run` : 8 |
| `C18.attached-manager.4` | `run` : 8 |
| `C19.custom-manager` | `mount` : 14 → `step-1.5` : 16 → `same-value` : 6 → `sibling-pass` : 6 → `step-1.0` : 14 |
