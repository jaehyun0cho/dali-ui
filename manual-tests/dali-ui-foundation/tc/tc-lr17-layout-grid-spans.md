# LR17. Layout: Grid spans

이 TC는 신규 Layout suite의 **Grid spans** 검증입니다. 대응 소스는 `tc-lr17-layout-grid-spans.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR17`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `G05.span-spacing`: span2의 width는30+7+50=87, height20+5+30=55입니다. (1,1)에서 span2는 x37,y25,width127,height75입니다.
- `G06.auto-deficit`: AUTO floor20+30+gap10=60이며 spanning child100의 부족40을 두 AUTO에20씩 더합니다. width40/50입니다.
- `G09.align.0..3`: cell100×60 안20×10을 START/CENTER/END/FILL로 배치합니다. CENTER=(40,25), END=(80,50), FILL=100×60입니다.
- `G10.padding-margin`: content112×62에서 margin6×14를 빼면106×48, 시작점(3+2,7+6)입니다.
...
span 범위의 내부 gap, AUTO 부족량 보정과 cell alignment를 별도 독립 식으로 검사합니다.
...
Profile `diagnostic / release`: 7 scenario, 7 action, 36 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `G05.span-spacing` | `run` : 8 |
| `G06.auto-deficit` | `run` : 8 |
| `G09.align..0` | `run` : 4 |
| `G09.align..1` | `run` : 4 |
| `G09.align..2` | `run` : 4 |
| `G09.align..3` | `run` : 4 |
| `G10.padding-margin` | `run` : 4 |
