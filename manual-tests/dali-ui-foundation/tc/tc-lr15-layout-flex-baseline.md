...
- 각 scenario의 필수 검사는17행입니다. a.line-spacing/b.line-spacing=0을 먼저 확인하고 a.local-baseline/b.local-baseline 실제값을 각각 renderedOffsetY+firstAscender와0.01 이내로 비교하십시오. 이 고정 단일 line·spacing0 fixture에서 glyph의 raw line-local y만 읽어0을 baseline으로 보고하면 이 수치 검사가 실패해야 합니다. 서로 다른 local baseline(차이>1), 동일 첫 glyph index, 큰 font의 큰 advance, 한 line·glyph 준비 조건을 먼저 검사합니다. raw ascender를 visual baseline이라고 가정하지 않습니다.
...
Profile `diagnostic / release`: 2 scenario, 2 action, 34 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `F14.align-items` | `run` : 17 |
| `F14.align-self` | `run` : 17 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR15 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
