# LR15. Layout: Flex baseline

이 TC는 신규 Layout suite의 **Flex baseline** 검증입니다. 대응 소스는 `tc-lr15-layout-flex-baseline.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR15`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `F14.align-items`, `F14.align-self`를 각각 실행하십시오. 고정 font `DejaVu Sans`, text `Hx`, fontSize18/36의 실제 Label을 같은 flex line에 놓습니다. fontconfig는 suite의 `res/layout-validation/fonts.conf`와 고정 font 파일을 사용하십시오. assets.json의 파일 hash가 일치해야 합니다.
- actual baseline은 이미 계산된 text line/glyph/render offset의 const 진단 snapshot입니다. `firstBaseline`에 child snapshot의 parent-local y를 더한 두 값이 0.01 이내에서 같아야 합니다.
- 각 scenario의 필수 검사는 17행입니다. a.line-spacing/b.line-spacing=0을 먼저 확인하고 a.local-baseline/b.local-baseline 실제값을 각각 renderedOffsetY+firstAscender와 0.01 이내로 비교하십시오. 이 고정 단일 line·spacing0 fixture에서 glyph의 raw line-local y만 읽어 0을 baseline으로 보고하면 이 수치 검사가 실패해야 합니다. 서로 다른 local baseline(차이>1), 동일 첫 glyph index, 큰 font의 큰 advance, 한 line·glyph 준비 조건을 먼저 검사합니다. raw ascender를 visual baseline이라고 가정하지 않습니다.
- 현재 구현이 BASELINE을 START처럼 배치하면 baseline equality가 실제 FAIL이어야 합니다. known failure, xfail, 다른 alignment 통과로 이 실패를 PASS로 치환하지 마십시오.
- font 누락/대체/준비 실패는 환경 또는 observer 실패이며 baseline 지원 PASS로 기록하지 마십시오.
...
공개 FlexAlign::BASELINE의 의미를 실제 rendered text baseline equality로 검증합니다. observer는 Text::View::GetGlyphs/GetGlyphsUncached를 호출하지 않고 이미 준비된 값을 bounded copy합니다.
...
Profile `diagnostic / release`: 2 scenario, 2 action, 38 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `F14.align-items` | `run` : 19 |
| `F14.align-self` | `run` : 19 |
