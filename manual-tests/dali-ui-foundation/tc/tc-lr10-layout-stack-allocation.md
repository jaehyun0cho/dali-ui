# LR10. Layout: Stack allocation

이 TC는 신규 Layout suite의 **Stack allocation** 검증입니다. 대응 소스는 `tc-lr10-layout-stack-allocation.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR10`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `S01.horizontal/vertical`: 전체 주축 300에서 fixed 60과 두 gap 20을 빼면 220입니다. weight 1:2는 220/3, 440/3이며 시작점은 0, 60+10=70, 70+220/3+10=80+220/3입니다. 마지막 시작점에도 두 번째 gap10을 더하며 마지막 child의 끝은 80+220/3+440/3=300이어야 합니다. 세 rect 12행을 검사하십시오. vertical은 두 축을 교환합니다.
- `S04.cross.0.3`: cross extent 60, intrinsic 10에 START/CENTER/END/FILL을 적용하면 y=0/25/50/0, height=10/10/10/60입니다.
- `S05.margin-gap-overflow`: 첫 child의 margin 포함 주축은 35, spacing 7이므로 둘째 x=42입니다. container 40 밖으로 나오는 배치를 임의 clamp하지 않습니다.

- `S04.fill-fixed-child`: FILL 정렬은 요청 크기가 WRAP_CONTENT인 child만 cross 축으로 확장합니다. 100×60 stack에서 고정 20×10 child는 (0,0,20,10)을 유지하고, 폭 30·높이 WRAP(min 5) child는 (20,0,30,60)으로 채웁니다.
...
`stack-layout-manager.cpp`의 non-weight/weight 분리, 주축/교차축 배치와 `StackLayoutParams` 계약을 검사합니다.
...
Profile `diagnostic / release`: 8 scenario, 8 action, 56 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `S01.horizontal` | `run` : 12 |
| `S01.vertical` | `run` : 12 |
| `S04.cross.0` | `run` : 4 |
| `S04.cross.1` | `run` : 4 |
| `S04.cross.2` | `run` : 4 |
| `S04.cross.3` | `run` : 4 |
| `S04.fill-fixed-child` | `run` : 8 |
| `S05.margin-gap-overflow` | `run` : 8 |
