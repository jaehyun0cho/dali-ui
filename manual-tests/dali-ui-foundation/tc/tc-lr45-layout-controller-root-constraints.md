# LR45. Layout: Controller root constraints

이 문서는 [tc-lr45-layout-controller-root-constraints.cpp](tc-lr45-layout-controller-root-constraints.cpp)의 신규 TC만 실행하는 절차입니다. 이전 Layout TC의 실행 결과를 이 TC의 증거로 합산하지 마십시오.
...
2. 검증 artifact의 `manual-tests/dali-ui-foundation/bin/manual-test-dali-ui-foundation`을 실행하고 stdout을 손실 없이 저장하십시오. launcher에서 `LR45. Layout: Controller root constraints`를 선택하십시오. 격리 build인 경우 해당 artifact 안의 binary를 실행하십시오.
3. `LR45.Reset run`을 한 번 눌러 새 run ID를 기록하십시오. 이후 실패를 지우기 위해 Reset하지 마십시오. 버튼 표시는 `Reset run`이며 Actor name과 accessibility name은 앞의 TC ID를 포함합니다.
4. 각 scenario에서 `LR45.Run scenario`를 누르면 남은 step이 순서대로 자동 실행됩니다(step 하나씩 진행하려면 `LR45.Run step`). `LR_ACTION`의 scenario/step/required_checks가 아래 순서와 일치하는지 먼저 확인하십시오. 각 step이 끝나면 `LR_READY`와 그 step의 모든 행이 자동 출력됩니다.
...
6. 각 step의 모든 검사 행은 완료 시 stdout에 자동 출력되며 HUD에는 요약과 실패 행만 표시되므로 행 수와 숫자는 stdout에서 빠짐없이 수집하십시오. 모든 step이 끝난 뒤에만 `LR45.Next scenario`를 누르거나, `LR45.Run all`로 남은 scenario를 이어서 자동 실행하십시오. 총 scenario 수는 5개입니다. `Run all` 뒤에는 마지막 `LR_RESULT`의 `completed_scenarios`가 `required_scenarios`와 같아질 때까지 기다리십시오. 그 전의 `FAILING`/`INCOMPLETE`는 종결 verdict가 아니므로 실행을 중단하지 마십시오. 완료 후 `< Back`으로 나가 `LR_CASE_END`의 `completed_scenarios`가 `required_scenarios`와 같은지 확인하십시오.

추가 창 제목은 `LR45 root constraints` 또는 `LR45 resize`입니다. 조작 버튼과 결과는 원래 manual-test launcher 창에 남아 있습니다. 추가 창이 앞에 나타나면 원래 창의 고유 `LR45.Run scenario`/`LR45.Run step` 식별자로 대상을 선택하십시오. 추가 창에 같은 위치를 클릭하여 다음 단계가 실행되었다고 간주하지 마십시오.

## 단계와 필수 관측 수

`CT01.constraint-0`~`3`: 각각 `run` 8개입니다. `CT01.real-window-resize`: `create` 4개 → `resize` 4개입니다. `resize`는 추가 창의 `SetPositionSize(40,40,400,300)`이 실제 창 크기 변경으로 반영되어야 완료됩니다. 창이 실제로 리사이즈되는 환경에서도 `observer.timeout`이 관측될 수 있으며(개발 환경에서 `xwininfo`로 리사이즈를 확인했는데도 timeout), 원인은 아직 확정되지 않았습니다. 그러므로 window manager 능력 문제로 단정하지 말고 환경·timing 의존으로 기록한 뒤 다른 host/타깃에서 재실행해 판정하십시오. 어떤 경우에도 timeout을 PASS로 기록하지 마십시오. 자세한 근거는 `layout-validation-coverage.md`의 환경/timing 의존 항목을 보십시오.
...
- 합성 Measure producer는 고정 (80,40)을 반환하는 plain View입니다. built-in manager에 계측 callback을 덮어쓰지 않습니다. margin=(3,7,5,11), requested position=(11,13)이므로 root 위치는 (14,18)입니다.

- mode 0: requested=(300,200), producer constraint=(290,184), 배치 크기=(80,40)입니다. mode 1: parent Actor=(250,180), MATCH_PARENT constraint/배치=(240,164)입니다. mode 2: parent 없음, Window=(320,240), constraint/배치=(310,224)입니다. mode 3: parent Actor 크기 0, constraint/배치=(0,0)이며 Window 크기로 대체하면 FAIL입니다.

- 실제 Window resize scenario는 별도 Window의 MATCH_PARENT root=(0,0,320,240)을 확인한 후 OS 크기를 400×300으로 변경합니다. 해당 Window의 fence에서 root=(0,0,400,300)을 확인하십시오. fence가 완료되지 않으면 환경·timing 의존(원인 미확정)으로 기록하고 PASS로 간주하지 마십시오.
...
- 모든 필수 action이 실행되고 expected checks와 실제 checks가 일치하며, 전체 5개 scenario가 완료되고 누적 failures=0이어야 합니다. 마지막 페이지 또는 마지막 scenario만 PASS인 것은 TC PASS가 아닙니다.
...
Profile `diagnostic / release`: 5 scenario, 6 action, 40 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `CT01.constraint-0` | `run` : 8 |
| `CT01.constraint-1` | `run` : 8 |
| `CT01.constraint-2` | `run` : 8 |
| `CT01.constraint-3` | `run` : 8 |
| `CT01.real-window-resize` | `create` : 4 → `resize` : 4 |
