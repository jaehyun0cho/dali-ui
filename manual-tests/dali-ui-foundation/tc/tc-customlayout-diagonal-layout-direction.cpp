...
{
/**
 * Diagonal arrangement callbacks that cooperate with LayoutDirection.
 *
 ...
 * Standalone children are skipped by both callbacks.
 */
class DiagonalDirectionCallbacks
{
public:
...
/**
 * @brief Verifies that the framework mirrors ArrangeCallback output under RTL.
 *
 * Same content as CustomLayout: Diagonal, plus a 200x50 translucent-black
 * standalone button at the top-left that toggles the root's LayoutDirection
 * between LEFT_TO_RIGHT and RIGHT_TO_LEFT on click.  The callbacks always
 * compute in a LEFT_TO_RIGHT frame and never see the mirroring; the standalone
 * button is excluded from it and stays at the top-left.
 *
 * Ported from samples/customlayout/customlayout-diagonal-layout-direction-example.cpp.
 */
class TcCustomLayoutDiagonalLayoutDirection : public ManualTest::TestCase, public ConnectionTracker
{
public:
  Dali::String GetName() const override
  {
    return "CustomLayout: Diagonal LayoutDirection Toggle";
  }

  Dali::String GetDescription() const override
  {
    return "Framework mirrors ArrangeCallback output under RIGHT_TO_LEFT";
  }
  ...
  void OnEnter(View contentArea) override
  {
    ...
    mRoot = Layout::New();
    mRoot.SetMeasureCallback(MeasureCallback::New(&DiagonalDirectionCallbacks::OnMeasure));
    mRoot.SetArrangeCallback(ArrangeCallback::New(&DiagonalDirectionCallbacks::OnArrange));
...
REGISTER_MANUAL_TEST(TcCustomLayoutDiagonalLayoutDirection)
