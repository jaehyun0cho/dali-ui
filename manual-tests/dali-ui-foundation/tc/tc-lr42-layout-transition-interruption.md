...
4. 각 scenario에서 `LR42.Run scenario`를 누르면 남은 step이 순서대로 자동 실행됩니다(step 하나씩 진행하려면 `LR42.Run step`). `LR_ACTION`의 scenario/step/required_checks가 아래 순서와 일치하는지 먼저 확인하십시오. 각 step이 끝나면 `LR_READY`와 그 step의 모든 행이 자동 출력됩니다.
...
6. 각 step의 모든 검사 행은 완료 시 stdout에 자동 출력되며 HUD에는 요약과 실패 행만 표시되므로 행 수와 숫자는 stdout에서 빠짐없이 수집하십시오. 모든 step이 끝난 뒤에만 `LR42.Next scenario`를 누르거나, `LR42.Run all`로 남은 scenario를 이어서 자동 실행하십시오. 총 scenario 수는 8개입니다. `Run all` 뒤에는 마지막 `LR_RESULT`의 `completed_scenarios`가 `required_scenarios`와 같아질 때까지 기다리십시오. 그 전의 `FAILING`/`INCOMPLETE`는 종결 verdict가 아니므로 실행을 중단하지 마십시오. 완료 후 `< Back`으로 나가 `LR_CASE_END`의 `completed_scenarios`가 `required_scenarios`와 같은지 확인하십시오.
...
- Animator callback은 좌표를 쓰지 않으므로 replacement가 engine lastLerped를 사용했는지 판정할 수 있습니다. Spec은 pause/seek 후 `Run::AfterFrame`으로 해당 action의 새 `REFRESH_ONCE` render task 완료와 요청 progress 반영을 확인한 다음 actual rendered x65를 독립적으로 검사합니다. expected geometry 일치는 대기 조건에 포함하지 않습니다. 이 완료 신호는 scene 상태의 freshness 관측이며 Window의 최종 화면 합성 완료가 아닙니다. replacement start 누계=2, 취소된 finish=0, 자연 완료 뒤 finish 누계=1이어야 합니다.
...
- 모든 필수 action이 실행되고 expected checks와 실제 checks가 일치하며, 전체 8개 scenario가 완료되고 누적 failures=0이어야 합니다. 마지막 페이지 또는 마지막 scenario만 PASS인 것은 TC PASS가 아닙니다.
...
모든 LR42 fixture는 결과 Label의 View subtree 밖에 있는 같은 실제 Window의 최상위 root이며, `Run::AttachToWindow`가 fixture host의 현재 화면 위치(HUD 아래)에 배치합니다. root의 `SENSITIVE=false`로 겹치는 영역의 입력을 받지 않으며, focus를 바꾸지 않습니다. 결과 Label 갱신의 invalidation이 paused CHANGE 좌표를 재배치하여 다음 replacement의 from을 바꾸지 않도록 합니다. 이 TC의 8개 scenario와 196개 assertion, quarter-point expected x=65는 그대로 유지합니다. 공통 `NewState`/`Mount` 및 focus TC의 전제는 변경하지 않습니다.
...
Profile `diagnostic / release`: 8 scenario, 30 action, 196 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `TR08.spec-enter-to-change` | `mount` : 4 → `enter-quarter` : 9 → `successor-half` : 14 → `successor-finish` : 9 |
| `TR08.spec-enter-to-exit` | `mount` : 4 → `enter-quarter` : 9 → `successor-half` : 14 → `successor-finish` : 9 |
