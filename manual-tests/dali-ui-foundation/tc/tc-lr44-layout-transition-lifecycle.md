...
4. 각 scenario에서 `LR44.Run scenario`를 누르면 남은 step이 순서대로 자동 실행됩니다(step 하나씩 진행하려면 `LR44.Run step`). `LR_ACTION`의 scenario/step/required_checks가 아래 순서와 일치하는지 먼저 확인하십시오. 각 step이 끝나면 `LR_READY`와 그 step의 모든 행이 자동 출력됩니다.
...
6. 각 step의 모든 검사 행은 완료 시 stdout에 자동 출력되며 HUD에는 요약과 실패 행만 표시되므로 행 수와 숫자는 stdout에서 빠짐없이 수집하십시오. 모든 step이 끝난 뒤에만 `LR44.Next scenario`를 누르거나, `LR44.Run all`로 남은 scenario를 이어서 자동 실행하십시오. 총 scenario 수는 13개입니다. `Run all` 뒤에는 마지막 `LR_RESULT`의 `completed_scenarios`가 `required_scenarios`와 같아질 때까지 기다리십시오. 그 전의 `FAILING`/`INCOMPLETE`는 종결 verdict가 아니므로 실행을 중단하지 마십시오. 완료 후 `< Back`으로 나가 `LR_CASE_END`의 `completed_scenarios`가 `required_scenarios`와 같은지 확인하십시오.
...
- 모든 필수 action이 실행되고 expected checks와 실제 checks가 일치하며, 전체 13개 scenario가 완료되고 누적 failures=0이어야 합니다. 마지막 페이지 또는 마지막 scenario만 PASS인 것은 TC PASS가 아닙니다.
...
Profile `diagnostic / release`: 13 scenario, 39 action, 171 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `TR11.tick-removes-self` | `mount` : 4 → `mutating-tick` : 9 → `drain` : 6 |
| `TR11.tick-replaces-with-exit` | `mount` : 4 → `mutating-tick` : 14 → `drain` : 6 |
| `TR12.parent-last-handle-spec` | `mount` : 4 → `destroy-parent` : 12 → `late-finish-silent` : 3 |
| `TR12.parent-last-handle-animator` | `mount` : 4 → `destroy-parent` : 12 → `late-finish-silent` : 3 |
