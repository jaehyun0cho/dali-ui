# LR34. Layout: Window cleanup

이 TC는 신규 Layout suite의 **Window cleanup** 검증입니다. 대응 소스는 `tc-lr34-layout-window-cleanup.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR34`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
`L04.window-recreate`는 세 action을 순서대로 실행하십시오.

1. `secondary-window`: 별도 220×140 Window를 만들고 child37×19의 해당 Window snapshot을 기다립니다. main Window와 다른 handle이어야 합니다.
2. `remove-and-recreate-controller`: root를 detach하고 해당 controller를 Remove한 뒤 Get으로 다시 생성합니다. child width53으로 재부착하여 새 controller의 snapshot53×19를 검사합니다.
3. `detach-close`: root unparent, controller Remove, secondary Window Hide와 handle 해제를 수행합니다. main Window는 유지됩니다.

각 Window fence 제한은 10초입니다. Window 생성이 불가능한 display/backend이면 환경 실패를 기록하고 PASS하지 마십시오. Back/Reset 중에도 secondary Window/controller cleanup이 실행되어야 합니다.
...
Window별 controller lifecycle과 root detach/recreate 이후 실제 layout 처리를 검사합니다. 앱의 main Window는 제거하지 않습니다.
...
Profile `diagnostic / release`: 1 scenario, 3 action, 13 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `L04.window-recreate` | `secondary-window` : 5 → `remove-and-recreate-controller` : 5 → `detach-close` : 3 |
