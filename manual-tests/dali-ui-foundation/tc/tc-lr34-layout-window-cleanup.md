...
1. `secondary-window`: 별도220×140 Window를 만들고 child37×19의 해당 Window snapshot을 기다립니다. main Window와 다른 handle이어야 합니다.
...
각 Window fence 제한은10초입니다. Window 생성이 불가능한 display/backend이면 환경 실패를 기록하고 PASS하지 마십시오. Back/Reset 중에도 secondary Window/controller cleanup이 실행되어야 합니다.
...
Profile `diagnostic / release`: 1 scenario, 3 action, 13 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR34 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
