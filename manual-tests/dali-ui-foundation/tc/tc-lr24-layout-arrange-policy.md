...
- `K15.if-changed/always`: 유효한 Measure(100,50)를 먼저 수행한 상태에서 동일 Arrange3회의 producer 수는1/3입니다. Measure 전의 cache 미게시 조건은 LR07에서 별도로 검증합니다.
- `K16.always-descendant`: 한 child의 ALWAYS 때문에 parent가 경로를 다시 방문하되 clean sibling producer는 재실행하지 않습니다.
- `K16.policy-change`: Measure(100,50) 후 IF_CHANGED1회→ALWAYS2회→기본 callback2호출의 총 producer는3→4입니다. Arrange callback policy 교체는 Arrange를 무효화하며 기존 Measure cache를 유지합니다.
...
|---|---|
| `K15.if-changed` | `run` : 5 |
| `K15.always` | `run` : 5 |
| `K16.always-descendant` | `run` : 10 |
| `K16.policy-change` | `run` : 2 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR24 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
