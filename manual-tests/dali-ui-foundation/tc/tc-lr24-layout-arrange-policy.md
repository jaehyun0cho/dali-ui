# LR24. Layout: Arrange policy

이 TC는 신규 Layout suite의 **Arrange policy** 검증입니다. 대응 소스는 `tc-lr24-layout-arrange-policy.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR24`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `K15.if-changed/always`: probe를 부착한 뒤 형제 leaf만 바꾸는 pass를 두 번 더 유도합니다. IF_CHANGED는 producer 1회를 유지하고 ALWAYS는 pass마다 실행되어 2, 3회입니다. 렌더 rect는 (0,0,37,19)로 변하지 않습니다.
- `K16.always-descendant`: 한 child의 ALWAYS 때문에 parent가 replay 대신 경로를 다시 방문하되 clean sibling producer는 재실행하지 않습니다.
- `K16.policy-change`: 부착 후 producer 1회, ALWAYS로 교체하면 invalidation으로 2회, 형제 pass에서 3회, 기본 callback(IF_CHANGED)으로 교체하면 4회, 이후 형제 pass에서는 4회를 유지합니다. Arrange callback policy 교체는 Arrange를 무효화하며 기존 Measure cache를 유지합니다.
...
ALWAYS는 해당 producer와 조상 경로의 캐시 재사용 정책을 바꾸며 clean sibling까지 무조건 실행시키지는 않습니다.
...
Profile `diagnostic / release`: 4 scenario, 13 action, 43 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `K15.if-changed` | `mount` : 4 → `second-pass` : 1 → `third-pass` : 5 |
| `K15.always` | `mount` : 4 → `second-pass` : 1 → `third-pass` : 5 |
| `K16.always-descendant` | `mount` : 8 → `extra-pass` : 10 |
| `K16.policy-change` | `mount` : 1 → `set-always` : 1 → `always-pass` : 1 → `set-if-changed` : 1 → `if-changed-pass` : 1 |
