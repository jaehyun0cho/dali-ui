...
- `G12.span.0/1/2/4294967295`: setter span0은1로 normalize합니다. span1은30×20, 나머지 큰 span은 남은 정의 수2로 제한되어87×55입니다.
...
정상 first/last index와 span 최소1은 필수 계약입니다. 초과 index/span은 공개 unsigned domain의 안전한 clamp 경로로 검증합니다.
...
Profile `diagnostic / release`: 8 scenario, 8 action, 44 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR18 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
