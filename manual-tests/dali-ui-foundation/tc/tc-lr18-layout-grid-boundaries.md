# LR18. Layout: Grid boundaries

이 TC는 신규 Layout suite의 **Grid boundaries** 검증입니다. 대응 소스는 `tc-lr18-layout-grid-boundaries.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR18`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `G12.index.0/1/4294967295`: first cell=(0,0,30,20), last와 초과 index는(37,25,50,30)입니다. index는 마지막 정의로 제한합니다.
- `G12.span.0/1/2/4294967295`: setter span0은 1로 normalize합니다. span1은 30×20, 나머지 큰 span은 남은 정의 수 2로 제한되어 87×55입니다.
- `G13.definition-clear`: 정의 삭제 후 초과 index라도 implicit 한 cell100×60을 사용합니다.

- `G13.definition-clear`는 `defined`((0,0,30,20)) → `cleared`(정의 제거 후 (0,0,100,60))의 두 step입니다.
...
정상 first/last index와 span 최소 1은 필수 계약입니다. 초과 index/span은 공개 unsigned domain의 안전한 clamp 경로로 검증합니다.
...
Profile `diagnostic / release`: 8 scenario, 9 action, 44 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `G12.index.0` | `run` : 4 |
| `G12.index.1` | `run` : 4 |
| `G12.index.4294967295` | `run` : 4 |
| `G12.span.0` | `run` : 6 |
| `G12.span.1` | `run` : 6 |
| `G12.span.2` | `run` : 6 |
| `G12.span.4294967295` | `run` : 6 |
| `G13.definition-clear` | `defined` : 4 → `cleared` : 4 |
