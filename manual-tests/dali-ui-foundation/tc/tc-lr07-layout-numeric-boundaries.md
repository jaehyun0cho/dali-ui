# LR07. Layout: Numeric boundaries

이 TC는 신규 Layout suite의 **Numeric boundaries** 검증입니다. 대응 소스는 `tc-lr07-layout-numeric-boundaries.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR07`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `C20.measure-tolerance`: probe(37×19)와 형제 leaf를 128×100 stage에 부착합니다. `mount` 뒤 stage 폭을 128.0005로 요청하면 setter가 sub-epsilon 변화를 무시하므로 형제 leaf의 폭만 바꿔 pass를 유도하고 producer 횟수가 그대로여야 합니다(`below-epsilon-hit`). 128.002는 반영되어 producer +1입니다(`above-epsilon-miss`). probe의 minimum width를 200으로 두고 stage 폭 120으로 pass하면 producer가 받은 폭은 200(`normalized-producer-input`)이고, 이어서 130으로 바꿔도 정규화 key가 같아 hit입니다(`normalized-hit`).
- `C20.pixel-setter`: pixel setter는 0.001 미만 변화(128.0005)를 무시하고 128.002는 저장합니다. 부착 후 렌더 폭은 128.002입니다.
- `C20.exact-params`: weight 0→0.0005는 모드 변경이며 정확히 저장되고 pass에서 producer가 +1이어야 합니다.
- `C20.requested-position-epsilon`: 요청 x 100으로 렌더된 probe에 100 바로 다음 float 값(100+2^-17≈100.0000076)을 요청하면 requested position setter의 ranged epsilon(|x|<200에서 100·FLT_EPSILON≈1.19e-5) 안이라 무시되어(형제 pass 유도 후) arrange producer가 그대로이고 렌더 x는 100입니다. 이어서 100.002를 요청하면 이 step 시작 시점 대비 producer +1이고 렌더 x는 100.002(허용오차 0.001)입니다. 128 크기에서는 float 1 ULP(≈1.5e-5)가 이미 epsilon을 넘으므로 sub-epsilon 값이 존재하지 않습니다.
- `C20.first-pass-cache`: 첫 controller pass에서 measure/arrange producer가 각각 정확히 1회 실행되고, 형제만 바꾼 두 번째 pass는 두 cache 모두 hit이라 횟수가 1로 유지됩니다.
...
`FloatEqual`을 사용하는 pixel/constraint 경로와 exact params/Arrange 경로를 분리합니다. 모든 nextafter에 producer 증가를 요구하지 않습니다. Arrange cache 비교는 유효한 Measure가 선행된 상태와 선행되지 않은 상태를 분리합니다. 근거는 internal/views/view/view-data-impl.cpp의 Arrange cache publish 조건(mMeasureCacheValid)입니다.
...
Profile `diagnostic / release`: 5 scenario, 13 action, 37 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C20.measure-tolerance` | `mount` : 4 → `below-epsilon` : 1 → `above-epsilon` : 1 → `normalized-minimum` : 1 → `normalized-hit` : 1 |
| `C20.pixel-setter` | `run` : 6 |
| `C20.exact-params` | `mount` : 4 → `mode-change` : 3 |
| `C20.requested-position-epsilon` | `mount` : 4 → `sub-epsilon` : 2 → `above-epsilon` : 2 |
| `C20.first-pass-cache` | `mount` : 6 → `unrelated-pass` : 2 |
