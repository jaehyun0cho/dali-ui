...
- Measure constraint 128→128.0005는 cache hit, 128→128.002는 miss입니다. min=200이면 raw 120/130 모두 200으로 정규화되어 hit입니다.
- pixel setter도 0.001 미만 변화는 저장값 128을 유지합니다.
- weight 0→0.0005는 모드 변경이며 정확히 저장하고 invalidation해야 합니다.
- C20.exact-arrange는 먼저 Measure(200,100)로 유효한 measure cache를 만든 뒤 검증합니다. Arrange x를 nextafter(128,200)로 변경하면 producer가 실행되고 그 float가 정확히 적용되어야 합니다. 이후 같은 slot 반복은 cache hit입니다. changed-producer, exact-position, unchanged-producer의3행을 확인하십시오.
- C20.unmeasured-arrange는 Measure 전 동일한 slot의 Arrange를 두 번 호출하여 producer 횟수1→2를 확인합니다. 첫 Measure 결과37×19를 확인한 뒤 Arrange 횟수3, 동일 slot 반복 후에도3이어야 합니다. unmeasured.first-producer/repeat-producer, first-valid-measure.width/height, measured.first-producer/repeat-cache의6행을 확인하십시오. Measure 전에 계산한 Arrange는 valid measure cache가 없으므로 재사용 가능한 entry를 게시하지 않아야 합니다.
...
Profile `diagnostic / release`: 5 scenario, 5 action, 18 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C20.measure-tolerance` | `run` : 4 |
| `C20.pixel-setter` | `run` : 2 |
| `C20.exact-params` | `run` : 3 |
| `C20.exact-arrange` | `run` : 3 |
| `C20.unmeasured-arrange` | `run` : 6 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR07 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
