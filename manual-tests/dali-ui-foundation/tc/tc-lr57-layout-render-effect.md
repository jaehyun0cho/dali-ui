...
`EFFECT.target-resize`는 별도 Window의 실제 offscreen render task/framebuffer texture metadata를 읽습니다. owner의 GaussianBlurEffect radius8, downscale1, blurOnce=true로 시작합니다.
...
Profile `diagnostic / release`: 1 scenario, 6 action, 37 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
