# LR57. Layout: Render effect layout

이 TC는 신규 Layout suite의 **Render effect layout** 검증입니다. 대응 소스는 `tc-lr57-layout-render-effect.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR57`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
`EFFECT.target-resize`는 별도 Window의 실제 offscreen render task/framebuffer texture metadata를 읽습니다. owner의 GaussianBlurEffect radius8, downscale1, blurOnce=**false**로 시작합니다. one-shot(blurOnce=true)에서는 단일 렌더가 끝나는 즉시 `OnRenderFinished`가 blur task를 제거하고 source task의 framebuffer를 비우므로, 아래 texture 관측이 그 해제와 경합합니다(빠른 GPU에서는 항상 0으로 관측). REFRESH_ALWAYS로 두어 source framebuffer가 유지되는 상태를 읽습니다.

| action | owner rect / source texture | 필수 행 수 |
|---|---|---:|
| mount | owner120×80, texture120×80 | 8 |
| resize | owner160×100, texture160×100 | 8 |
| zero-size | owner0×0, effect 비활성, source task0 | 6 |
| restore-odd-size | owner73×41, texture73×41 | 8 |
| downscale-half | texture floor(73×.5)×floor(41×.5)=36×20 | 4 |
| clear-effect | owner effect empty, 비활성, source task0 | 3 |

source task는 `GetSourceActor()==owner`이며 유효 FrameBuffer가 있는 task만 셉니다. blur 중간 task의 texture를 owner source로 잘못 선택하지 마십시오. texture width/height와 task 수는 정수 exact입니다. Window fence는 geometry settle 기준이며 GPU pixel golden 완료를 의미하지 않습니다. 이 TC의 목적은 resize 후 stale/off-by-one render target 검출입니다.
...
`view-data-impl.cpp`의 size 변경→RenderEffect refresh와 `render-effect-impl.cpp::UpdateTargetSize`, `gaussian-blur-effect-impl.cpp`의 source texture floor/max 계산을 public RenderTask/FrameBuffer/Texture reader로 검사합니다.
...
Profile `diagnostic / release`: 1 scenario, 6 action, 37 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `EFFECT.target-resize` | `mount` : 8 → `resize` : 8 → `zero-size` : 6 → `restore-odd-size` : 8 → `downscale-half` : 4 → `clear-effect` : 3 |
