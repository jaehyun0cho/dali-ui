# LR33. Layout: Reparent and offscene

이 TC는 신규 Layout suite의 **Reparent and offscene** 검증입니다. 대응 소스는 `tc-lr33-layout-reparent-offscene.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR33`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `L01.reparent`: left a20+gap7+b30, right c40 상태에서 b를 right로 옮깁니다. left20×10, right(40+5+30)×20, b x45입니다. 양쪽 논리 child 수와 parent identity도 검사합니다.
- `L02.detached-measure`: child37×19의 requested position(7,9)이 parent44×28을 만듭니다. detach 후 parent0×0, child37×19이며 재부착 후 44×28입니다.

- `L01.reparent`는 `mount`(left (0,0,57,15), right (0,50,40,20)) → `reparent`(b를 right로 이동: old 20×10, new 75×20, b 렌더 (45,0,30,15))이고, `L02.detached-measure`는 `mount`(44×28) → `remove`(0×0) → `detached`(off-scene child의 직접 Measure 37×19: detached 계약 자체가 검증 대상) → `reattach`(44×28, child (7,9,37,19))입니다.
...
scene 밖에서도 public synchronous 계산이 유효하고 reparent가 양쪽 parent cache 및 child 관계를 갱신하는지 검사합니다.
...
Profile `diagnostic / release`: 2 scenario, 6 action, 44 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `L01.reparent` | `mount` : 8 → `reparent` : 12 |
| `L02.detached-measure` | `mount` : 6 → `remove` : 6 → `detached` : 2 → `reattach` : 10 |
