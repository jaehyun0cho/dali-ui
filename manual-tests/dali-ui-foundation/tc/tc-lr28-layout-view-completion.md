...
`Read result`와 페이지 이동이 이전 action의 frozen snapshot을 바꾸면 FAIL입니다. 다음 action의 required set은 새로 구성되며 서로 다른 Window episode의 child snapshot을 합치지 않습니다.
...
Profile `diagnostic / release`: 1 scenario, 4 action, 25 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
