# LR28. Layout: View completion

이 TC는 신규 Layout suite의 **View completion** 검증입니다. 대응 소스는 `tc-lr28-layout-view-completion.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR28`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
`L03.fenced-snapshots`에는 순서대로 네 action이 있습니다.

1. `mount`: child a(0,0,20,10), b(20,0,30,15)의 View 신호를 받고 Window settle fence에서 고정합니다. 각 child 신호 수는 양수여야 합니다.
2. `resize-child`: 먼저 다음 fence를 arm한 뒤 a width40을 적용합니다. snapshot a width40, b x40입니다.
3. `no-op-setter`: 같은 width40의 setter는 새 신호를 기다리지 않습니다. 동기 신호 수가 그대로인지 검사합니다.
4. `direct-arrange`: manual Arrange(7,9,40,10)는 target을 바꾸되 관리 pass의 completion 신호를 즉시 emit하지 않습니다.

HUD 갱신이 이전 step의 frozen snapshot을 바꾸면 FAIL입니다. 다음 action의 required set은 새로 구성되며 서로 다른 Window episode의 child snapshot을 합치지 않습니다.
...
View LayoutFinishedSignal은 parent-local final target이며 실제 Window fence보다 먼저 전달되는 계약을 검사합니다.
...
Profile `diagnostic / release`: 1 scenario, 4 action, 25 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `L03.fenced-snapshots` | `mount` : 10 → `resize-child` : 8 → `no-op-setter` : 2 → `direct-arrange` : 5 |
