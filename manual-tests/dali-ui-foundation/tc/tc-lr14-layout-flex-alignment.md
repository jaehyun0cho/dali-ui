# LR14. Layout: Flex alignment

이 TC는 신규 Layout suite의 **Flex alignment** 검증입니다. 대응 소스는 `tc-lr14-layout-flex-alignment.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR14`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `F04.justify.0.5`: children20/40, free80입니다. START/END/CENTER/BETWEEN/AROUND/EVENLY의 x쌍은 (0,20)/(80,100)/(40,60)/(0,100)/(20,80)/(80/3,20+160/3)입니다.
- `F05.align-self.0.4`: parent END, child AUTO/START/END/CENTER/STRETCH에서 y50/0/50/25/0, 마지막 height60입니다. sibling은 y50을 유지합니다.
- `F01.direction.0.3`: 100 주축에 20/30을 순서대로 배치합니다. reverse x80/50이며 COLUMN은 축을 교환합니다.
- BASELINE은 별도 LR15의 실제 glyph baseline assertion으로 검증하십시오.

- `F01.rtl.{0,1}`: RIGHT_TO_LEFT의 ROW/ROW_REVERSE에서 logical 배치는 LTR과 같고(ROW: a (0,0,20,10), b (20,0,30,10); ROW_REVERSE: a (80,…), b (50,…)) 렌더 x는 100−x−width로 mirror됩니다(ROW: a 80, b 50; ROW_REVERSE: a 0, b 20). controller가 보고한 `*.reported` snapshot도 mirror된 최종 rect와 같아야 합니다(pre-RTL 값이면 FAIL). 16행.
...
- `F12.overflow.3/4/5`: shrink0인 30+30을 width40에 배치합니다. SPACE_BETWEEN은 x0/30, SPACE_AROUND/EVENLY는 x−10/20입니다. 부족 공간을 음수 gap으로 분배하지 않습니다.

## 근거와 검증 경계

justify, alignSelf 우선순위, 네 방향의 좌표식을 검사합니다. BASELINE을 START fallback으로 합격 처리하지 않습니다.
...
Profile `diagnostic / release`: 20 scenario, 20 action, 176 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `F04.justify.4` | `run` : 8 |
| `F04.justify.5` | `run` : 8 |
| `F05.align-self.0` | `run` : 8 |
| `F05.align-self.1` | `run` : 8 |
| `F05.align-self.2` | `run` : 8 |
| `F05.align-self.3` | `run` : 8 |
| `F05.align-self.4` | `run` : 8 |
| `F01.direction.0` | `run` : 8 |
| `F01.direction.1` | `run` : 8 |
| `F01.direction.2` | `run` : 8 |
| `F01.direction.3` | `run` : 8 |
| `F01.rtl.0` | `run` : 16 |
| `F01.rtl.1` | `run` : 16 |
| `F12.overflow.3` | `run` : 8 |
| `F12.overflow.4` | `run` : 8 |
| `F12.overflow.5` | `run` : 8 |
