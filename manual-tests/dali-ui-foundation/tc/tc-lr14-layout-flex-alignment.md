...
- `F01.direction.0..3`: 100 주축에20/30을 순서대로 배치합니다. reverse x80/50이며 COLUMN은 축을 교환합니다.
...
- `F12.overflow.3/4/5`: shrink0인30+30을 width40에 배치합니다. SPACE_BETWEEN은 x0/30, SPACE_AROUND/EVENLY는 x−10/20입니다. 부족 공간을 음수 gap으로 분배하지 않습니다.
...
Profile `diagnostic / release`: 18 scenario, 18 action, 144 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
