# LR13. Layout: Flex lines

이 TC는 신규 Layout suite의 **Flex lines** 검증입니다. 대응 소스는 `tc-lr13-layout-flex-lines.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR13`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `F02.wrap.normal/reverse.100`: 40+60이 정확히 100이면 같은 line, 세 번째 child는 다음 line입니다. normal y=0/0/20, reverse y=15/15/0입니다.
- 폭 99이면 첫 line은 40, 둘째 line은 60+30이며 normal y=0/10/10, reverse y=20/0/0입니다.
- `F13.content.0.3`: line height10+20, free cross70입니다. START/CENTER/END의 첫 y는 0/35/70, STRETCH는 각 line에 35를 추가하여 둘째 y45입니다.

- `F03.wrap-measure`: 100×80 stage 안 wrapping flex(WRAP_CONTENT)에 40/60/30 폭 child를 두면 line [40+60=100], [30]으로 나뉘어 measured (100,35)=(max line main, cross 합 20+15)이고 렌더도 (0,0,100,35)입니다.
...
- `F02.wrap-grow-lines`: 100×60 WRAP flex에 basis 40(grow 1)/50(grow 1)/30(grow 2)/30(grow 0)/120(shrink 1)을 두면 줄은 [40,50] [30,30] [120]이고 줄마다 free space를 나눕니다: 45/55, 70/30, 그리고 120은 혼자인 줄에서 100으로 shrink합니다. 렌더 (0,0,45,10) (45,0,55,10) (0,10,70,20) (70,10,30,20) (0,30,100,10)입니다.

## 근거와 검증 경계

wrap의 strict 초과 조건, line별 크기, WRAP_REVERSE와 alignContent를 검증합니다.
...
Profile `diagnostic / release`: 10 scenario, 10 action, 106 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `F02.wrap.normal.100` | `run` : 12 |
| `F02.wrap.normal.99` | `run` : 12 |
| `F02.wrap.reverse.100` | `run` : 12 |
| `F02.wrap.reverse.99` | `run` : 12 |
| `F03.wrap-measure` | `run` : 6 |
| `F13.content.0` | `run` : 8 |
| `F13.content.1` | `run` : 8 |
| `F13.content.2` | `run` : 8 |
| `F13.content.3` | `run` : 8 |
| `F02.wrap-grow-lines` | `run` : 20 |
