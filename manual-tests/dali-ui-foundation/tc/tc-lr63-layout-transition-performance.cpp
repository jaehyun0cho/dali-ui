...
void MeasureSample(Run& r, std::shared_ptr<PerformanceState> state)
{
  ...
  {
    r.Equal("performance.samples", state->sample, SAMPLE_COUNT);
    r.Equal("performance.operations-per-sample", UPDATE_OPERATIONS, 9);
    ...
    for(unsigned mode = 0; mode < 5; ++mode)
      for(unsigned count : {1u, 32u, 128u})
                            ...
                            [mode, count](Run& r)
        {
          ...
          r.RequireExternalVerification("The Release OFF samples and paired comparison are also required for a timing verdict.");
        } }
#else
                          {"release-samples", (SAMPLE_COUNT + 1u) * GeometryChecks(count) + 2u, [mode](Run& r)
        {
          auto s = r.State<PerformanceState>();
          if(mode == 1)
          {
            ...
            r.Equal("performance.samples", SAMPLE_COUNT, 31);
            r.Equal("performance.operations-per-sample", DORMANT_OPERATIONS, 128);
