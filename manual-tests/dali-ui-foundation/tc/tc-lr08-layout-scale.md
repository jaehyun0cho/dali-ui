...
- R03: scalable=false 상태에서 NaN/-1/+Inf 설정을 시도하고 저장값 1 유지 여부를 검사한 뒤 즉시 1로 복원합니다. +Inf를 허용하면 실제 robustness FAIL입니다.
...
- `C20.scale-exact-key`: Window에 등록된 root의 scale1→1.0005는 constraint epsilon과 달리 정확한 key 변경입니다. producer가1회 증가하고 scale key=1.0005여야 합니다. 같은 scale의 다음 호출은 hit입니다.
- `C13.actor-scale-repair`: diagnostics가 제공한 실제 property index의 값을9로 손상시킵니다. 다음 cache-hit Measure가 producer 재실행 없이 actor scale1과 sync bit를 복원해야 합니다. index를 숫자로 하드코딩하지 않습니다.
...
Profile `diagnostic / release`: 8 scenario, 8 action, 43 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C13.scale.1.000000` | `run` : 6 |
| `C13.scale.1.250000` | `run` : 6 |
| `C13.scale.1.500000` | `run` : 6 |
| `C13.scale.2.000000` | `run` : 6 |
| `C14.K12.master-switch` | `run` : 6 |
| `R03.scale-finite` | `run` : 3 |
| `C20.scale-exact-key` | `run` : 5 |
| `C13.actor-scale-repair` | `run` : 5 |
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR08 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
