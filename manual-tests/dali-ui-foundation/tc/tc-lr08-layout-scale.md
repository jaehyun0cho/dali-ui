# LR08. Layout: Scale inheritance

이 TC는 신규 Layout suite의 **Scale inheritance** 검증입니다. 대응 소스는 `tc-lr08-layout-scale.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR08`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- C13: parent DISABLED=200×100, INHERIT child=20×10, ENABLED child=(30s,15s)를 s=1/1.25/1.5/2에서 검사합니다.
- C14/K12: scale 2에서 74×38, master OFF에서 37×19, ON에서 74×38입니다.
- R03: scalable=false 상태에서 NaN/-1/+Inf/subnormal(`denorm_min`) 설정을 시도하고 저장값 1 유지 여부를 검사한 뒤 즉시 1로 복원합니다. +Inf나 subnormal을 허용하면 실제 robustness FAIL입니다(layout이 scale로 나누므로 subnormal은 1/scale이 +Inf가 됩니다).

- `C13.scale.{s}`는 scale 비적용 stage(400×200)에 DISABLED parent를 부착하고 measured 3쌍과 inherit/enabled child의 렌더 rect((0,0,20,10), (0,0,30s,15s))를 검사합니다(14행). `C14.K12.master-switch`는 `enabled`(74×38) → `disabled`(37×19) → `restored`(74×38)의 세 step이며 각 step은 measured 2행 + 렌더 4행입니다.
- `C20.scale-exact-key`(`mount` → `sub-epsilon-scale` → `same-scale`)와 `C13.actor-scale-repair`(`mount` → `corrupt-and-repair`)는 관측 hook을 사용하며, hook이 모든 빌드에 포함되므로 아래 계약표대로 항상 선언·실행됩니다.
...
- `C20.scale-exact-key`: Window에 등록된 root의 scale1→1.0005는 constraint epsilon과 달리 정확한 key 변경입니다. producer가 1회 증가하고 scale key=1.0005여야 합니다. 같은 scale의 다음 호출은 hit입니다.
- `C13.actor-scale-repair`: diagnostics가 제공한 실제 property index의 값을 9로 손상시킵니다. 다음 cache-hit Measure가 producer 재실행 없이 actor scale1과 sync bit를 복원해야 합니다. index를 숫자로 하드코딩하지 않습니다.

## 근거와 검증 경계

effective scale 상속·override·master switch와 scale 변경 cache를 검사합니다. 잘못된 전역 scale을 실제 scene에 전파하지 않고 setter guard를 검사합니다.
...
Profile `diagnostic / release`: 8 scenario, 13 action, 96 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `C13.scale.1.000000` | `run` : 14 |
| `C13.scale.1.250000` | `run` : 14 |
| `C13.scale.1.500000` | `run` : 14 |
| `C13.scale.2.000000` | `run` : 14 |
| `C14.K12.master-switch` | `enabled` : 6 → `disabled` : 6 → `restored` : 6 |
| `R03.scale-finite` | `run` : 4 |
| `C20.scale-exact-key` | `mount` : 4 → `sub-epsilon-scale` : 4 → `same-scale` : 1 |
| `C13.actor-scale-repair` | `mount` : 5 → `corrupt-and-repair` : 4 |
