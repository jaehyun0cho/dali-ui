...
- `G02.star`: 330−50−20=260을1:2로 나눕니다. column width=50,260/3,520/3입니다.
- `G03.auto`: intrinsic40의 AUTO 뒤 gap10, STAR150입니다.
- `G08.zero-star-overflow`: container40보다 fixed60이 커도 fixed track을 축소하지 않습니다. STAR0은0, 시작점67입니다.
- `G04.implicit`: 정의가 없으면100×60 한 cell입니다. 정의를20/30으로 교체한 후30×20입니다.
...
Profile `diagnostic / release`: 5 scenario, 5 action, 48 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR16 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
