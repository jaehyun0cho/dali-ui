# LR16. Layout: Grid tracks

이 TC는 신규 Layout suite의 **Grid tracks** 검증입니다. 대응 소스는 `tc-lr16-layout-grid-tracks.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR16`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- `G01.absolute`: rows20/30와 gap5, columns40/50와 gap7입니다. 좌표는 각 이전 track과 gap의 합입니다.
- `G02.star`: 330−50−20=260을 1:2로 나눕니다. column width=50,260/3,520/3입니다.
- `G03.auto`: intrinsic40의 AUTO 뒤 gap10, STAR150입니다.
- `G08.zero-star-overflow`: container40보다 fixed60이 커도 fixed track을 축소하지 않습니다. STAR0은 0, 시작점 67입니다.
- `G04.implicit`: 정의가 없으면 100×60 한 cell입니다. 정의를 20/30으로 교체한 후 30×20입니다.

- `G01.absolute`는 wrapping grid의 measured (97,55)=(40+7+50, 20+5+30) 2행을 추가로 검사합니다(14행). `G04.implicit`는 `implicit`((0,0,100,60)) → `explicit`(tracks {20},{30}: (0,0,30,20))의 두 step입니다.
- `G14.mixed-span-deficit`: 열 {Absolute(30), Auto}, 열 간격 10에서 폭 100의 2열 span child는 부족분 60을 Auto 열에만 배분합니다. 기본 정렬 FILL이므로 렌더 rect는 cell과 같아 span은 (0,0,100,20)(30+10+60, 행 높이 20)이고 행 1 열 1의 child는 Auto 열 폭 60을 그대로 보여 (40,20,60,20)입니다.
...
- `G15.wrap-minimum-star`: WRAP_CONTENT 폭 grid(min width 100, 열 {Absolute(30), Star(1)}, 높이 20)는 measured 100×20이고 star 열이 arrange 시점의 남은 70을 받아 a (0,0,30,20), b (30,0,70,20)입니다.

## 근거와 검증 경계

track 종류별 budget, implicit track, zero factor, definition 변경에 따른 cache invalidation을 검사합니다.
...
Profile `diagnostic / release`: 7 scenario, 8 action, 68 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.
...
| `G02.star` | `run` : 12 |
| `G03.auto` | `run` : 8 |
| `G08.zero-star-overflow` | `run` : 8 |
| `G04.implicit` | `implicit` : 4 → `explicit` : 4 |
| `G14.mixed-span-deficit` | `run` : 8 |
| `G15.wrap-minimum-star` | `run` : 10 |
