...
`I09.hit-clipped-layout`는 별도240×160 Window 안에서 실제 DALi touch dispatch를 사용합니다. 외부 마우스를 fixture Window로 이동하지 마십시오. 각 action이 `SceneHolder::FeedTouchPoint`로 DOWN/UP을 전달하고 event 처리를100ms 기다립니다.

- `mount`: root Window-local(20,20,100,80), child root-local(80,20,60,40)입니다. child는 root 오른쪽을40만큼 넘습니다.
- `inside-hit`: Window 좌표(110,50)는 child local(10,10)이며 실제 hitActor=child, DOWN1회입니다.
- `outside-clipped`: (140,50)는 child 안이지만 root clip 밖이므로 child DOWN 증가0입니다.
...
각 검사 수는 순서대로8/4/1/4/8/4입니다. local 좌표 tolerance0.05, 정수 counter와 actor identity는 exact입니다. 실제 event가 전달되지 않으면 timeout/불일치로 실패하며 geometry 계산만으로 hit 성공을 추정하지 않습니다.
...
Profile `diagnostic / release`: 1 scenario, 6 action, 29 checks. diagnostics가 필수인 action은 OFF의 capability 실패를 PASS로 처리하지 마십시오.
...
서버가 사전 고정한 manifest와 SHA256으로 `python3 layout-validation-tools.py check-log LOG --tc LR56 --profile diagnostic --manifest MANIFEST --manifest-sha256 PIN`을 실행하십시오. Release에서는 profile을 바꾸십시오. `PIN`을 candidate 출력에서 즉석 생성하지 마십시오. 계약이 다르면 별도 검토가 필요합니다. 외부 검증이 필요한 TC의 `--external` 결과는 pending이며 exit 1입니다. 해당 MD의 외부 증거가 없으면 전체 PASS가 아닙니다.
