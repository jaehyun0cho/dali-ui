# LR56. Layout: Hit and clip

이 TC는 신규 Layout suite의 **Hit and clip** 검증입니다. 대응 소스는 `tc-lr56-layout-hit-clip.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR56`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
`I09.hit-clipped-layout`는 별도 240×160 Window 안에서 실제 DALi touch dispatch를 사용합니다. 외부 마우스를 fixture Window로 이동하지 마십시오. 각 action이 `SceneHolder::FeedTouchPoint`로 DOWN/UP을 전달하고 event 처리를 100ms 기다립니다.

- `mount`: root Window-local(20,20,100,80), child root-local(80,20,60,40)입니다. child는 root 오른쪽을 40만큼 넘습니다.
- `inside-hit`: Window 좌표(110,50)는 child local(10,10)이며 실제 hitActor=child, DOWN1회입니다.
- `outside-clipped`: (140,50)는 child 안이지만 root clip 밖이므로 child DOWN 증가 0입니다.
- `unclipped-hit`: clip을 DISABLED로 바꾸고 한 event 구간을 기다립니다. 같은(140,50)의 hitActor=child, local(40,10), DOWN1회입니다.
- `enlarge-clip`: root width140과 clip 복원 이후 child는 같은 rect를 유지합니다.
- `newly-visible-hit`: (140,50)는 넓어진 clip 안이므로 local(40,10), DOWN1회입니다.

각 검사 수는 순서대로 8/4/1/4/8/4입니다. local 좌표 tolerance0.05, 정수 counter와 actor identity는 exact입니다. 실제 event가 전달되지 않으면 timeout/불일치로 실패하며 geometry 계산만으로 hit 성공을 추정하지 않습니다.
...
Absolute parent-local geometry와 core clip/hit integration을 실제 Window touch event로 교차 검증합니다. 위치 표는 입력으로 고정되며 현재 hit 결과를 expected에 사용하지 않습니다.
...
Profile `diagnostic / release`: 1 scenario, 6 action, 29 checks. 이 표에 없는 scenario는 컴파일된 계약에 선언되지 않으며 실행·판정 대상이 아닙니다.

| Scenario ID | 순서대로 실행할 action ID : required_checks |
|---|---|
| `I09.hit-clipped-layout` | `mount` : 8 → `inside-hit` : 4 → `outside-clipped` : 1 → `unclipped-hit` : 4 → `enlarge-clip` : 8 → `newly-visible-hit` : 4 |
