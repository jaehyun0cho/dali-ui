# LR04. Layout: LayoutParams value semantics

이 TC는 신규 Layout suite의 **LayoutParams value semantics** 검증입니다. 대응 소스는 `tc-lr04-layout-params-values.cpp`입니다. 기존 TC의 실행 결과를 사용하지 않습니다.

## 실행 조건

- foundation manual-test 앱에서 `LR04`를 검색하고 표시명이 일치하는 TC에 진입하십시오.
...
- 각 scenario는 원본 params를 View에 설정한 뒤 원본을 수정합니다. View의 getter snapshot은 설정 당시 값을 유지해야 합니다.
- Stack snapshot 수정만으로 View가 바뀌면 FAIL이며 다시 SetLayoutParams했을 때만 weight=3이어야 합니다.
- Flex는 basis=17/grow=2/shrink=3/CENTER, Grid는 row=2/column=3/span=4×5/END, Absolute는 (1,2,30,40)/X_PROPORTIONAL입니다.

- `C16.rendered-copy`는 같은 params handle을 두 child에 설정한 뒤 원본 handle의 weight를 9로 바꿔도 저장된 복사본(1:1)이 렌더를 결정함을 확인합니다. `mount`: a (0,0,100,50), b (100,0,100,50); `reassign-a`(a weight 3 재설정): a (0,0,150,50), b (150,0,50,50).
...
`SetLayoutParams`의 독립 복사와 `TryGetLayoutParams`의 snapshot 계약을 검사합니다. 공유 handle 변경에 의존하지 않습니다.
...
|---|---|
| `C16.stack-copy` | `run` : 6 |
| `C16.rendered-copy` | `mount` : 8 → `reassign-a` : 8 |
| `C16.flex-copy` | `run` : 6 |
| `C16.grid-copy` | `run` : 6 |
| `C16.absolute-copy` | `run` : 6 |
