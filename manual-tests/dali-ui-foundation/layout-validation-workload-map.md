...
- `RequireExternalVerification`이 있는 action의 로컬 CHECK 통과는 전체 performance PASS가 아닙니다. 일치하는 work-budget 검사·timing 표본·고정 reference·환경·반복 raw sample 증거가 필요합니다.
- timing 대상은 Release, coverage OFF이며 timing step 동안 capture를 열지 않습니다. coverage/profiler 실행 시간은 Release baseline에 합치지 않습니다.

## 현재 선언된 계약 크기

다음 수치는 컴파일된 앱의 `BuildScenarios()` 선언에서 생성한 manifest 합계입니다. 관측 hook이 항상 포함되므로 profile별 구분은 없습니다. 실행 결과가 아닙니다.

| TC | scenario / action / required checks | 검사 계약 |
|---|---|---|
| LR52 | 50 / 137 / 2929 | 기존2131 + same-count/partial/notify542 + animation 중 resize256; SC.match-constraints17행 |
| LR59 | 12 / 84 / 85544 | manager 4×N{16,128,512}; timing step 4개(220+4N)와 work-budget step 3개(42+4N) |
| LR60 | 3 / 18 / 18129 | N{16,128,512}; timing step 3개(220+4N)와 work-budget step 3개(45+4N) |
| LR61 | 7 / 7 / 4146 | flat 4×771(256 cycle×3 + 최종 렌더 3) + depth 34/258/770 |
| LR62 | 21 / 34 / 1513 | 기존145 + spec/animator × CHANGE/ENTER/EXIT × 시작 전 등록 유무의 historical snapshot1368 |
| LR63 | 30 / 60 / 141970 | mode 0~4×N{1,32,128}; timing `TR15` 15개(mount+release-samples)와 work-budget `TR15W` 15개(mount+work-count) |
| LR64 | 18 / 30 / 7502 | 기존 timing/font7226 + 실제 RecyclerView correctness companion276 |

## Diagnostics bridge 결정과 검사

| source function / decision | 신규 scenario → action | 실제 assertion ID / 독립 불변조건 | 관측과 범위 |
|---|---|---|---|
| `F/internal/views/view/view-data-impl.cpp:9061` `RegisterNode`: empty/ID0/duplicate ID 거부, 같은 handle 재등록, ID 교체 | LR62 `D01.registry → run` | `id.empty-rejected`, `id.zero-rejected`, `id.duplicate-rejected`, `id.same-object-idempotent`, `id.a=101`, `id.replaced-value=102`, `id.released-value-reusable` | snapshot nodeId와 명시 입력 101/102 대조; ID를 object 주소에서 계산하지 않음 |
...
| `Record` inactive return / capacity full / 정상 append (`view-data-impl.cpp:8932`) | LR62 `D03.overflow-reset → run` | `overflow.flag`, `.canaries`, `.capacity=2`, `.stored-count=2`, `.total-not-truncated`, `.sequence-first=1`, `.sequence-second=2`, `.epoch-first/second=6203` | 앞뒤 canary와 붙어 있는 2 event buffer; totalEvents는 stored count보다 커야 함 |
...
| `LayoutTransitionDispatcher::GetLayoutTestSnapshot/GetLayoutTestAnimation` (`layout-transition-dispatcher.cpp:2815` 이후) | LR62 `D05.readonly-snapshots → run` | `readonly.transition-fields`, `.animation-absent` | D05는 정지 상태의 반복 read만 검사; D07의12개 scenario는 active spec/animator CHANGE/ENTER/EXIT의 모든 snapshot field를 독립 expected와 비교하고 late registration/clear/rebind 후 historical owner와 live node/parent ID를 구분 |
...
### 비활성 hook 비용과 observer allocation 증거

| 결정/접점 | 연결된 검사 | 증거의 정확한 의미 |
|---|---|---|
| `internal/layouts/layout-test-diagnostics.h`의 `IsCapturing()` 검사를 인자 목록 **밖**에 두는 hook macro; `gCaptureActive` 하나가 capture 활성의 유일한 출처 | LR62 `D00.profile-always-on → run`; LR62 MD의 symbol/ABI/disassembly 절차 | 앱 내부 검사만으로는 hot-path 비용을 증명하지 않음. 변경 이전 Release binary와 비교하여 diagnostics export 12개가 존재하고 dispatcher 멤버는 hidden이며 각 hook 자리가 load 한 번 + 조건 분기 한 번인지 binary artifact를 별도 보존 |
| reader/capture 함수 실행 구간만 allocator interposition | LR62 MD의 observer allocation 보조 실험 | valid/invalid readers와 Begin/End 호출의 함수별 calls/completed/allocator entry를 별도 profiler로 관측. dlsym/로그/TC UI allocation은 scope 밖. 재귀·중첩 scope·hook binding positive control 필요 |
| `readonly.capture-empty` 및 named storage event가 0 | LR62 D05, LR59/60 work rows | Layout event가 없다는 것만 보장; malloc0을 증명하지 않음. direct mmap/custom arena/asynchronous thread 등 profiler 미관측 경로까지 no allocation이라고 일반화하지 않음 |
...
## LR59: controller 계산 phase

구현은 `T/tc-lr59-layout-compute-performance.cpp`의 `TimePhase`, `Benchmark`, `CheckWork`, `ObserveImmediate`와 `T/layout-validation-workloads.h`입니다. workload는 `StageOnWindow`로 부착되며 pass는 `LayoutController::ProcessLayouts()`를 통합니다. N={16,128,512}, W=10N−2, H=16, leaf i=(10i,0,8,16)이 독립 기대값입니다.

| phase/action | 실행 경로 | 독립 검증 |
|---|---|---|
| `construction` | fixture 생성만 timing; 부착/pass는 밖 | 요청값 scalar3+2N, 이후 모든 leaf geometry |
| `first_pass` / `work-first_pass` | 새 root의 controller pass1회 | immediate12+6N; measure/arrange producer·publish=N+1, ancestor=N, root drain1; Grid만 추가 measure hit=N |
| `warm_pass` / `work-warm_pass` | 형제 Nudge 후 pass32회 | measure/arrange enter·hit=32, producer0, replay=32(N+1), root drain32, replay children storage32회 |
| `idle_pass` / `work-idle_pass` | clean Window의 pass4096회 | workload의 work event/storage0 및 전체 geometry 불변 |

`WorkCapture::Check`는 protocol, event 및 명시 storage site의 exact budget을 검사합니다. first pass의 storage는 `MeasureStorage`와 `ArrangeStorage`를 합산하며 최초 geometry write 수는 변경 전 Actor와 독립 expected의 차이에서 계산합니다. workload의 등록 node만 계수하므로 HUD 전체 비용을 포함한 값으로 해석하지 마십시오.

매 sample의 `sample.immediate.*`, `sample.timer_positive.*`, `sample.geometry.*`를 검사합니다. 최종 `rendered.*`는 timing/capture 구간 밖에서 새 render task 완료 뒤 current scene geometry를 읽습니다. 모든 leaf를 검사하며 frame timeout 때 event-side 결과로 대체하지 않습니다. exact 행 수와 storage 공식은 [LR59 TC](tc/tc-lr59-layout-compute-performance.md)의 검토된 계약을 따릅니다. 삭제된 `first_measure/first_arrange/warm_measure/warm_arrange` phase는 현재 metric이 아닙니다.
...
|---|---|---|
| `nN → work-sparse` / timing `sparse` | 가운데 child 하나만 변경, C=1 | `work.measure.enter=N+1`, producer/publish=C+1, hit=N−C; `work.arrange.enter=N+1`, producer/publish=C+1, hit=N−C; `work.replay.visit=N−C`; `work.geometry.write=C`; `work.invalidate.measure=2C`; `work.ancestor.visit=2C+1` |
...
| 모든 work-budget action | 반환과 현재 geometry | `output.measured.width/height=W/16`, `output.arranged`=(0,0,W,16), `geometry.i`의 height만 해당 input15/16, `geometry.visited=N`, `.mismatches=0`, `.maximum_error=0` |
| 모든 timing action | sample마다 15↔16 변경; same-value는 16 고정 | `sample.immediate.j.values=12+6N`, `.mismatches=0`, `.maximum_error=0`; `sample.timer_positive.j`; `sample.geometry.j.visited/mismatches/maximum_error`; fresh frame 이후 `rendered.i` 전체 Rect |

timing step 각 sample의 setter와 controller pass만 측정 구간에 포함합니다. expected height 갱신·검사·로그는 interval 뒤이며 반환·geometry를 복구하는 추가 layout은 호출하지 않습니다. 불필요한 producer/storage 증가가 timing에서 작게 보이더라도 work-budget exact 실패를 무시하지 마십시오.

## LR64: 복합 계산 workload의 독립 oracle

구현: `T/tc-lr64-layout-complex-performance.cpp`의 `ComplexFixture::Prepare/Mutate/Calculate/Observe`, `BenchmarkComplex`입니다. 각 family는 N16/N128 두 scenario이고 action은 단일 `timing`입니다(text-reflow는 그 앞에 `font-identity`). state s는 0/1을 번갈아 바꾸며 expected 구성은 timing 밖, setter+계산은 안, 관측은 직후입니다.

모든 sample j=0…30의 실제 assertion은 `sample.immediate.j.values/.mismatches/.maximum_error`이며 `sample.timer.j`와 raw sample이 함께 나옵니다. 마지막 sample은 `final.root`와 `final.node.i`를 개별 Rect로 남깁니다. **LR64는 producer/hit/storage exact budget을 추가로 계수하지 않으며**, family별 geometry와 짝지은 timing을 검사합니다. 단순 manager exact work budget은 LR59/60이 담당합니다.

| family / source decision | 독립 fixture·식 | scalar 수 / 개별 행 범위 |
|---|---|---|
| `deep-dirty-nN`: Stack `Measure/Arrange`, child invalidation의 ancestor 경로 | depth N, 폭 120, leaf height20+s; 모든 N ancestor WRAP height20+s; 각 local origin0 | root+N descendant, values=10+4N, D=4(N+1) |
...
| `text-reflow-nN`: `LabelImpl::OnMeasure`의 width-dependent height, `GetHeightForWidth :2363`, 완료 text model | 고정 DejaVu Sans24, A N개, CHARACTER wrap, W25/40에서 1/2 glyph per line; H=29×ceil(N/columns). 고정 asset font identity는 앞 action `font-identity` | values10, D4. text engine shaping 전체·다국어 전체 또는 async image decode 성능을 이 workload의 범위로 주장하지 않음 |

## 인접 soak·transition 성능 연결

| source decision | 신규 scenario/action | 실제 assertion ID / 범위 |
|---|---|---|
| `ReplayCachedArrange`가 변조된 Actor position/size를 cached result로 복구 | LR61 `stack/flex/grid/absolute → run` | 128child,256cycle; index=(cycle×37) mod128에 x=−123.25,width1을 쓴 뒤 `cycle.j.visited=128`, `.mismatches=0`, `.maximum_error=0`;16cycle마다 명시 invalidate. heap leak 판정은 TC MD의 10번 실제 재진입·cleanup/RSS/PSS/allocation-profile 증거가 별도 필요 |
...
| 같은 transition workload의 계산 시간 | LR63 `TR15` scenario의 `release-samples` | `performance.warmup.*`, `performance.sample-j.*` 모든 logical target, samples31, operations128(dormant) 또는 9(update). active animation Actor 중간값 대신 완료 logical target을 읽음; 실제 timer tick 비용 전체를 dormant work-count로 대신하지 않음 |
...
| `ScrollViewLayoutManager::Arrange :106`의 MATCH 대입/Arrange-owned 재측정 | 같은 scenario | `owner.arrange`=(0,0,100,80), `content.arranged`=(11,7,w,h); `arrange.constraint.width/height`; `arrange.measure.producers`=mixed 축만 2, 양축MATCH/양축WRAP1. natural 축 재측정 constraint도 240/200으로 exact 대조 |
...
| padding origin과 MATCH inner extent, axis별 scroll clamp | `SC.axis-padding-margin.{0…2} → mount/maximum/resize/padding-change` | `viewport`(arrange된 actor rect), `content`, `scroll.x/y`, `cached-viewport.width/height`(ScrollViewImpl이 저장한 viewport), `scrollable.width/height`, `direction`. content width=Horizontal ?240:viewportW−start−end; height=Vertical ?200:viewportH−top−bottom; origin=(start−scrollX,top−scrollY) |
...
Page signal buffer 검사 ID는 각 action의 `page.signal.buffer`(고정 64 event, overflow없음), `page.signal.page`, `page.signal.count`입니다. Notify의 임시 range(page100×expected count400/300)는 실제 마지막 부분 page를 포함하는 최종 extent350/250과 다릅니다. source 내부 임시값을 최종 geometry oracle로 사용하지 않습니다.
...
| 첫 실제 layout에서 expected page와 표시 위치 일치 | 같은 scenario `attach-and-settle` | `pending.settled.*`, `pending.visible-identity/local/origin`; before current1/scroll100,after0/0,empty−1/0. pending navigation 유실은 expected를 0으로 바꾸지 않고 FAIL로 유지 |
...
| measurement window 분리 | LR59/60/64 sample41중 앞 10 제외,31 raw sample; phase별 operations를 함께 기록 | parser가 timer_floor_ns 이상 유한 duration, 동일 operations/scenario/action을 검사 |
| 독립 reference pin·조건 일치 | `compare`의 app/library/fixture hash, assets, CPU affinity/governor, resolution/DPI, compiler flags, `capture_active_during_timing=false`(hook은 모든 빌드에 있으므로 빌드 옵션이 아니라 실행 조건입니다. 구 이름 `diagnostics`를 쓰면 `compare`가 거부합니다)/`coverage=false`/Release | 같은 fixture revision, 동일 기기/환경. baseline 부재·조건 불일치는 PASS가 아님. app×library2×2 실험도 logical workload가 같아야 함 |
...
| work regression | LR59/60 work-budget exact event/storage budget 중 하나라도 다르면 FAIL | timing의 통계 결과와 독립적으로 보고; allocation 원인 분석은 별도 profiler 증거 |
